#!/usr/bin/env python3
"""生成 Phi-LPF 最新 offdiagonal seed 到 tuple-fields 的同步证书。

用法示例：
  python3 experiments/prime_matrix_phi_lpf_latest_offdiagonal_seed_tuple_sync_router.py
  python3 -m json.tool docs/monograph/prime-matrix-phi-lpf-latest-offdiagonal-seed-tuple-sync-router.json
...
SLUG = "prime-matrix-phi-lpf-latest-offdiagonal-seed-tuple-sync"
OUT_LEDGER = DATA / f"{SLUG}-ledger.json"
OUT_JSON = DOCS / f"{SLUG}-router.json"
OUT_MD = DOCS / f"{SLUG}-router.md"

LATEST_SEED_CERT = DOCS / "prime-matrix-phi-lpf-latest-semiprime-seed-diagonal-sync-router.json"
    ...
    return [
        {
            ...
            "meaning": "offdiagonal seed 表拆成已闭合的无符号 tuple 字段和仍开放的 signed/trace/ExactUV 字段。",
        },
        {
        ...
        },
        {
            "from": COMMON_PACKET,
            "to": source_atoms(),
            "meaning": "common packet 不再作为自证终点；最新基底已把它携带为 source 三原子。",
        },
        {
            "from": OFFDIAG_SEED,
            "to": (
                f"{OFFDIAG_SIGNED_FORMULA} AND {OFFDIAG_ORIENTATION} AND "
                f"{OFFDIAG_EXACTUV} plus {INTERNAL_TRANSITION} and carried source atoms"
            ),
            "meaning": "最新 offdiagonal seed 黑箱被剥去 LPF/Phi 无符号层后，只剩真正 signed 来源字段。",
        },
    ]
...
def build_rows(
    ...
    """生成最新 offdiagonal seed tuple 同步判定表。"""
    atoms = source_atoms()
    return [
        row(
            "LatestOffDiagonalSeedHardpointImported",
            latest.get("next_primary_attack_target") == OFFDIAG_SEED,
            False,
            "上一层最新 semiprime seed 同步已把新 seed 硬点定位到 offdiagonal 表。",
            OFFDIAG_SEED,
        ),
        row(
            ...
            True,
            "既有 tuple-fields 证书可直接作用在最新 offdiagonal seed 入口。",
        ...
        row(
            ...
            True,
            "LPF/Phi 已用尽：它只给支撑、tuple 字段和容量，不能产生 signed coefficient。",
        ...
        row(
            ...
            False,
            "diagonal/common packet 义务在最新基底中仍以 source 三原子携带，尚未由本步证明。",
            atoms,
        ),
        row(
            ...
            False,
            "最新 offdiagonal seed 表被收窄为 source tuple signed formula 与配套字段。",
        ...
        row(
            ...
            False,
            False,
            "本步只同步 offdiagonal seed 的 tuple-fields 剥离；未证明三命题无条件闭合。",
            f"{OFFDIAG_SIGNED_FORMULA} AND {OFFDIAG_ORIENTATION} AND {OFFDIAG_EXACTUV} AND {INTERNAL_TRANSITION} AND {atoms}",
        ),
    ]


def build_certificate() -> dict[str, Any]:
    """组装最新 offdiagonal seed tuple 同步证书。"""
    latest = load_json(LATEST_SEED_CERT)
    ...
    latest_basis = (
        ...
        f"OR {PDEC_SCOPE} OR {NEW_JOINT}) AND {EXACTUV_PAIR} AND {MODEL} AND {RATE} "
        f"AND {DSTRUCTURE}"
    )
    return {
        "certificate_type": "prime_matrix_phi_lpf_latest_offdiagonal_seed_tuple_sync_router",
        "status": "phi_lpf_latest_offdiagonal_seed_synced_to_tuple_fields_open",
        "same_theorem_target_preserved": True,
        "no_theorem_switch": True,
        "frontier_sync_only": True,
        "finite_evidence_not_used_as_global_proof": True,
        "latest_offdiagonal_seed_hardpoint_imported": rows[0]["closed"],
        "offdiagonal_tuple_fields_router_imported": rows[1]["closed"],
        "offdiagonal_source_tuple_bijection_synced": rows[2]["closed"],
        "offdiagonal_phi_tail_fiber_mass_synced": rows[3]["closed"],
        "offdiagonal_unsigned_tuple_fields_closed": rows[4]["closed"],
        "lpf_phi_unsigned_scope_exhausted_for_offdiag_seed": rows[4]["closed"],
        "source_atoms_carried_forward": rows[5]["closed"],
        "latest_basis_replaces_offdiag_seed_with_tuple_payload": rows[6]["closed"],
        ...
        "plain_conclusion": (
            "本步把最新 `PhiLPFOffDiagonalOrderedSemiprimeFirstSeedSignedTableBeforePushforward` "
            "接入既有 tuple-fields 证书。LPF/Phi 已关闭 offdiagonal source tuple 的 owner、first prime、"
            "q-rough tail 与 Phi fiber mass；因此 offdiagonal seed 口不再是未解析黑箱。"
            "剩余不能由 LPF/Phi 反推，必须正向提交 source tuple signed seed formula、orientation parity/"
            "branch side、ExactUV fixed pair/return tag，并保留 internal transition 与 source 三原子。"
        ),
    }
...
def render_markdown(cert: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "```text",
        f"latest_offdiagonal_seed_hardpoint_imported={fmt_bool(cert['latest_offdiagonal_seed_hardpoint_imported'])}",
