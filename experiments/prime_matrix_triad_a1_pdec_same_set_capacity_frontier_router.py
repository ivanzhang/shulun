...
DEFAULT_DIBFI_DI_RDN_SUBSTITUTION = (
    DOCS / "prime-matrix-triad-a1-dibfi-di-rdn-substitution-router.json"
)
DEFAULT_DIBFI_W4_PARAMETER_LEDGER = (
    DOCS / "prime-matrix-triad-a1-dibfi-w4-parameter-ledger-router.json"
)
...
DEFAULT_DIBFI_FULL_S_DISPERSION_ATOM = (
...
DEFAULT_DIBFI_EXACT_FULL_S_SOURCE_ENTROPY_REDUCTION = (
    DOCS
...
def build_frontier_rows(
    ...
    return [
        ...
        {
            "frontier": "A1DIBFINonAPObjectLedgerRouter",
            "status": "nonap_object_ledger_reduced_to_uncentered_wfd_no_projection_identity_open",
            "evidence": (
                f"非 AP-source 对象侧已剥离 APErrorRepresentation；"
                f"open_object_gates={dibfi_nonap_object_ledger['open_object_gates']}；"
                f"terminal_gap={dibfi_nonap_object_ledger['terminal_gap_after_router']}。"
            ),
        ...
        {
            "frontier": "A1DIBFIDIRDNSubstitutionRouter",
            "status": "di_rdn_substitution_reduced_to_current_wfd_maynard_w4_parameter_ledger_open",
            "evidence": (
                f"DI R/D/N 已代入 Maynard-W4 正规形；"
                f"open_substitution_gates={dibfi_di_rdn_substitution['open_substitution_gates']}；"
                f"terminal_gap={dibfi_di_rdn_substitution['terminal_gap_after_router']}。"
            ),
            "next_action": "证明当前 WFD off-diagonal 块逐项生成 W4 的 z/y/b/c 参数，并完成 J-bound 三项支配。",
        },
        {
            "frontier": "A1DIBFIW4ParameterLedgerRouter",
            "status": "w4_parameter_ledger_reduced_to_current_wfd_maynard_conditions_open",
            "evidence": (
                f"Maynard-W4 参数模板与 J-bound 简化已固定；"
                f"open_w4_gates={dibfi_w4_parameter_ledger['open_w4_gates']}；"
                f"terminal_gap={dibfi_w4_parameter_ledger['terminal_gap_after_router']}。"
            ),
            "next_action": "建立当前 WFD 到 N_May/R_May/S_May/M_May/Q_May 的翻译表，并证明一条对角、两条非对角条件。",
        },
        {
            "frontier": "A1DIBFIMaynardExponentConeRouter",
            "status": "maynard_w4_conditions_reduced_to_current_wfd_exponent_cone_open",
            "evidence": (
                f"Maynard-W4 三条件已合并为指数锥；"
                f"open_cone_gates={dibfi_maynard_exponent_cone['open_cone_gates']}；"
                f"terminal_gap={dibfi_maynard_exponent_cone['terminal_gap_after_router']}。"
            ),
        ...
        {
            "frontier": "A1DIBFIMaynardSCompressionBarrierRouter",
            "status": "maynard_s_common_inverse_direct_identification_rejected_open",
            "evidence": (
                f"朴素 S_common=S_May 已由指数锥排除；"
                f"open_barrier_gates={dibfi_maynard_s_compression_barrier['open_barrier_gates']}；"
                f"terminal_gap={dibfi_maynard_s_compression_barrier['terminal_gap_after_router']}。"
            ),
        ...
        {
            "frontier": "A1DIBFIMaynardSCompressionNoGoRouter",
            "status": "maynard_s_compression_map_rejected_full_s_window_open",
            "evidence": (
                f"完整 S 窗口下 Maynard-S 压缩已被排除；"
                f"open_nogo_gates={dibfi_maynard_s_compression_nogo['open_nogo_gates']}；"
                f"terminal_gap={dibfi_maynard_s_compression_nogo['terminal_gap_after_router']}。"
            ),
        ...
        {
            "frontier": "A1DIBFIFullSDispersionAtomRouter",
            "status": "full_s_dispersion_atom_reduced_to_external_dibfi_match_open",
            "evidence": (
                f"full-S 原子已精确化；"
                f"open_atom_gates={dibfi_full_s_dispersion_atom['open_atom_gates']}；"
                f"terminal_gap={dibfi_full_s_dispersion_atom['terminal_gap_after_router']}。"
            ),
            "next_action": "提交 ExternalFullSDIBFIAtomMatch，或继续攻无投影未中心化对象恒等式。",
        },
        {
            "frontier": "A1DIBFIExternalFullSMatchRouter",
            "status": "external_full_s_match_reduced_to_kls_specialization_open",
            "evidence": (
                f"外部 full-S 匹配已拆成 KLS 专门化；"
                f"open_match_gates={dibfi_external_full_s_match['open_match_gates']}；"
                f"terminal_gap={dibfi_external_full_s_match['terminal_gap_after_router']}。"
            ),
        ...
        {
            "frontier": "A1DIBFINewFullSTheoremInputRouter",
            "status": "new_full_s_theorem_input_reduced_to_full_s_nonap_wfd_kls_input_open",
            "evidence": (
                f"NewFullSTheoremInput 已压成唯一审稿级定理原子；"
                f"open_input_gates={dibfi_new_full_s_theorem_input['open_input_gates']}；"
                f"terminal_gap={dibfi_new_full_s_theorem_input['terminal_gap_after_router']}。"
            ),
        ...
        {
            "frontier": "A1DIBFIExactFullSSourceEntropyReductionRouter",
            "status": "exact_full_s_source_entropy_reduced_to_factor_support_package_open",
            "evidence": (
                f"exact full-S source entropy 已压成精确因子支撑包；"
                f"open_reduction_gates="
                f"{dibfi_exact_full_s_source_entropy_reduction['open_reduction_gates']}；"
                f"terminal_gap="
                f"{dibfi_exact_full_s_source_entropy_reduction['terminal_gap_after_router']}。"
            ),
            "next_action": (
                "证明 full-S non-AP 精确 u/v 因子支撑、balanced range 阈值与 "
        ...
        {
            "frontier": "A1DIBFIFullSSourceAntiAtomRouter",
            "status": "full_s_support_capacity_reduced_to_source_antiatom_or_external_open",
            "evidence": (
                f"支撑+容量兼容已压成 full-S non-AP source capacity 反原子合同；"
                f"open_antiatom_gates={dibfi_full_s_source_antiatom['open_antiatom_gates']}；"
                f"terminal_gap={dibfi_full_s_source_antiatom['terminal_gap_after_router']}。"
            ),
            "next_action": (
                "证明最终 source capacity measure 无 moving same-(u,v) 原子，"
...
def run(
    ...
    return {
        ...
        "source_hashes": {
            ...
            "a1_dibfi_di_rdn_substitution_json": file_sha256(
                dibfi_di_rdn_substitution_path
            ),
            "a1_dibfi_w4_parameter_ledger_json": file_sha256(
                dibfi_w4_parameter_ledger_path
            ),
            ...
            "a1_dibfi_full_s_dispersion_atom_json": file_sha256(
                dibfi_full_s_dispersion_atom_path
            ),
...
def main() -> None:
    ...
    parser.add_argument(
        "--dibfi-nonap-object-ledger-json",
        type=Path,
    ...
    parser.add_argument(
        "--dibfi-di-rdn-substitution-json",
        type=Path,
        default=DEFAULT_DIBFI_DI_RDN_SUBSTITUTION,
    )
    parser.add_argument(
        "--dibfi-w4-parameter-ledger-json",
        type=Path,
        default=DEFAULT_DIBFI_W4_PARAMETER_LEDGER,
    )
    parser.add_argument(
        "--dibfi-maynard-exponent-cone-json",
        type=Path,
    ...
    parser.add_argument(
        "--dibfi-maynard-s-compression-barrier-json",
        type=Path,
    ...
    parser.add_argument(
        "--dibfi-maynard-s-compression-nogo-json",
        type=Path,
    ...
    parser.add_argument(
        "--dibfi-full-s-dispersion-atom-json",
        type=Path,
        default=DEFAULT_DIBFI_FULL_S_DISPERSION_ATOM,
    )
    parser.add_argument(
        "--dibfi-external-full-s-match-json",
        type=Path,
    ...
    parser.add_argument(
        "--dibfi-new-full-s-theorem-input-json",
        type=Path,
    ...
    parser.add_argument(
        "--dibfi-completed-weight-spectral-gap-json",
        type=Path,
    ...
    parser.add_argument(
        "--dibfi-exact-full-s-source-entropy-reduction-json",
        type=Path,
    ...
    parser.add_argument(
        "--dibfi-full-s-source-antiatom-json",
        type=Path,
    ...
    result = run(
        ...
        dibfi_full_s_dispersion_atom_path=(
            args.dibfi_full_s_dispersion_atom_json
        ),
        dibfi_external_full_s_match_path=(
