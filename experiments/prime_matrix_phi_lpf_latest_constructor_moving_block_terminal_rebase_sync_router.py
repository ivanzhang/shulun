#!/usr/bin/env python3
"""生成 latest constructor moving-block terminal 的 rebase 同步证书。

用法示例：
  python3 experiments/prime_matrix_phi_lpf_latest_constructor_moving_block_terminal_rebase_sync_router.py
  python3 -m json.tool docs/monograph/prime-matrix-phi-lpf-latest-constructor-moving-block-terminal-rebase-sync-router.json
...
SLUG = "prime-matrix-phi-lpf-latest-constructor-moving-block-terminal-rebase-sync"
OUT_LEDGER = DATA / f"{SLUG}-ledger.json"
OUT_JSON = DOCS / f"{SLUG}-router.json"
OUT_MD = DOCS / f"{SLUG}-router.md"

CURRENT_FRESH_REBASE_CERT = (
    DOCS / "prime-matrix-phi-lpf-latest-constructor-fresh-joint-identity-taxonomy-rebase-sync-router.json"
)
OLD_MOVING_BLOCK_CERT = DOCS / "prime-matrix-phi-lpf-latest-constructor-moving-block-terminal-sync-router.json"
...
PRECAUCHY_ALPHA_TERMINAL_CERT = DOCS / "prime-matrix-phi-lpf-latest-precauchy-alpha-terminal-sync-router.json"
...
def build_rows(
    current_fresh: dict[str, Any],
    old_moving: dict[str, Any],
    moving: dict[str, Any],
    global_split: dict[str, Any],
    alpha_terminal: dict[str, Any],
) -> list[dict[str, Any]]:
    """生成 moving-block terminal rebase 判定表。"""
    current_moving_active = (
        current_fresh.get("next_primary_attack_target") == MOVING_BLOCK
        and current_fresh.get("fresh_joint_declaration_reduced_to_moving_block") is True
    )
    old_moving_reusable = (
        old_moving.get("target_input_before_router") == MOVING_BLOCK
        and old_moving.get("constructor_moving_block_unnamed_exit_removed") is True
        and old_moving.get("next_primary_attack_target") == PDEC_CLEAN_KLS
    )
    ...
    unnamed_exit_removed = (
        current_moving_active and old_moving_reusable and strict_router_closed and global_terminal_imported
    )
    return [
        row(
            "LatestRebasedFreshJointMovingBlockInputImported",
            current_moving_active,
            False,
            "上一层 rebase 已把 constructor fresh joint declaration 压到 actual moving-block/NC-BLK。",
            MOVING_BLOCK,
        ),
        row(
            "ExistingConstructorMovingBlockTerminalReusable",
            old_moving_reusable,
            False,
            "旧 constructor moving-block terminal 同步输入相同，可在新 rebase 前沿复用。",
...
def build_result() -> dict[str, Any]:
    """构造同步证书。"""
    current_fresh = load_json(CURRENT_FRESH_REBASE_CERT)
    old_moving = load_json(OLD_MOVING_BLOCK_CERT)
    ...
    rows = build_rows(current_fresh, old_moving, moving, global_split, alpha_terminal)
    return {
        "certificate_type": "prime_matrix_phi_lpf_latest_constructor_moving_block_terminal_rebase_sync_router",
        "status": "phi_lpf_latest_constructor_moving_block_terminal_rebased_open",
        ...
        "latest_rebased_fresh_joint_moving_block_input_imported": rows[0]["closed"],
        "existing_constructor_moving_block_terminal_reusable": rows[1]["closed"],
        "strict_actual_moving_block_router_imported": rows[2]["closed"],
        "counterexample_branch_guard_preserved": rows[3]["closed"],
        "global_pdec_sparse_split_imported": rows[4]["closed"],
        "current_materialized_terminal_frontier_exhausted": rows[5]["closed"],
        "precauchy_alpha_terminal_sync_agrees": rows[6]["closed"],
        "constructor_moving_block_unnamed_exit_removed": rows[7]["closed"],
        ...
        "latest_retained_basis_after_router": terminal_basis(),
        "structural_chain": [
            FRESH_JOINT_DECL,
            MOVING_BLOCK,
            f"{PDEC_CLEAN_KLS} AND {MODEL_GAP}",
        ...
        "plain_conclusion": (
            "本步把最新 rebase 后的 `ActualNoncanonicalMovingBlockSpreadNCBLKForCounterexampleBranchAndReturn` "
            "接入 strict moving-block 终端路由。该输入不能作为新的无名黑箱；全局拆分把它压到 "
            "`PDEC_CAP_OR_INTERNAL_CleanKLS_LargeSieve`，同时 `ExplicitModelGapAndFiniteDPRCLedger` "
            "仍独立保留。"
        ),
    }
...
def render_markdown(result: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "```text",
        f"latest_rebased_fresh_joint_moving_block_input_imported={fmt_bool(result['latest_rebased_fresh_joint_moving_block_input_imported'])}",
        f"existing_constructor_moving_block_terminal_reusable={fmt_bool(result['existing_constructor_moving_block_terminal_reusable'])}",
