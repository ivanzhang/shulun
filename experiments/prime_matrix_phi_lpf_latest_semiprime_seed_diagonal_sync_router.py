#!/usr/bin/env python3
"""生成 Phi-LPF 最新 semiprime seed 到 diagonal/offdiagonal 拆分的同步证书。

用法示例：
  python3 experiments/prime_matrix_phi_lpf_latest_semiprime_seed_diagonal_sync_router.py
  python3 -m json.tool docs/monograph/prime-matrix-phi-lpf-latest-semiprime-seed-diagonal-sync-router.json
...
SLUG = "prime-matrix-phi-lpf-latest-semiprime-seed-diagonal-sync"
OUT_LEDGER = DATA / f"{SLUG}-ledger.json"
OUT_JSON = DOCS / f"{SLUG}-router.json"
OUT_MD = DOCS / f"{SLUG}-router.md"

LATEST_EDGE_CERT = DOCS / "prime-matrix-phi-lpf-latest-edge-multiplier-slab-sync-router.json"
...
def sync_chain() -> list[dict[str, str]]:
    ...
    return [
        ...
        },
        {
            "from": COMMON_PACKET,
            "to": source_atoms,
            "meaning": "common packet 的自证环已被切断；最新前沿已把其非循环需求携带为 source 三原子。",
        },
        {
            "from": FIRST_SEED,
            "to": f"{OFFDIAG_SEED} plus carried source atoms",
            "meaning": "在最新保留基中，FIRST_SEED 的 diagonal 义务被已有 source 三原子吸收，剩余新 seed 口是 offdiagonal 表。",
        },
    ]


def build_rows(data: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    """生成最新 semiprime seed 同步判定表。"""
    latest = data["latest"]
    ...
    return [
        ...
        row(
            ...
            True,
            "既有 semiprime seed diagonal/offdiagonal 证书可作为 FIRST_SEED 的直接下游。",
        ...
        row(
            ...
            True,
            "common packet 不能用 signed-lane 固定点自证；最新前沿已把它携带为 source 三原子。",
            source_atoms,
        ),
        row(
            ...
            False,
            "在最新基中，FIRST_SEED 的 diagonal 义务由 source 三原子承接，新增 seed 缺口收窄到 offdiagonal 表。",
            OFFDIAG_SEED,
        ),
        row(
        ...
        row(
            ...
            False,
            False,
            "本步只同步 FIRST_SEED 的 diagonal/offdiagonal 拆分；未证明 offdiagonal seed、internal transition、source 三原子、ExactUV、模型、Rate 或 DStructure。",
            f"{source_atoms} AND {OFFDIAG_SEED} AND {INTERNAL_TRANSITION}",
        ),
    ]


def build_certificate() -> dict[str, Any]:
    """组装最新 semiprime seed diagonal 同步证书。"""
    data = {
        "latest": load_json(LATEST_EDGE_CERT),
    ...
    return {
        "certificate_type": "prime_matrix_phi_lpf_latest_semiprime_seed_diagonal_sync_router",
        "status": "phi_lpf_latest_semiprime_seed_synced_to_offdiag_and_source_atoms_open",
        "same_theorem_target_preserved": True,
        "no_theorem_switch": True,
        "frontier_sync_only": True,
        "finite_evidence_not_used_as_global_proof": True,
        "latest_first_seed_hardpoint_imported": rows[0]["closed"],
        "semiprime_diagonal_router_imported": rows[1]["closed"],
        "diagonal_offdiagonal_support_split_closed": rows[2]["closed"],
        "diagonal_private_escape_removed": rows[3]["closed"],
        "common_packet_cycle_guard_carried_forward": rows[4]["closed"],
        "latest_basis_replaces_first_seed_with_offdiag_seed": rows[5]["closed"],
        ...
        "plain_conclusion": (
            "本步把最新 `PhiLPFSemiprimeFirstEdgeSignedSeedTableBeforePushforward` "
            "接入既有 diagonal/offdiagonal 拆分。diagonal `(p,p)` 已由 square-base 线证明没有私有 "
            "signed 出口，并在最新前沿中由 base/source-packet 三原子承接；因此 FIRST_SEED 的新增 "
            "signed 缺口收窄为 `PhiLPFOffDiagonalOrderedSemiprimeFirstSeedSignedTableBeforePushforward`。"
            "这一步仍不证明 offdiagonal signed seed，也不证明 internal prime-adjoin transition；"
            "行/列命题仍未无条件证明。"
        ),
    }
...
def render_markdown(cert: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "```text",
        f"latest_first_seed_hardpoint_imported={fmt_bool(cert['latest_first_seed_hardpoint_imported'])}",
