#!/usr/bin/env python3
"""生成 Phi-LPF latest new-joint alpha 前沿到终端三原子的同步证书。

用法示例：
  python3 experiments/prime_matrix_phi_lpf_latest_new_joint_alpha_terminal_three_atoms_sync_router.py
  python3 -m json.tool docs/monograph/prime-matrix-phi-lpf-latest-new-joint-alpha-terminal-three-atoms-sync-router.json
...
SLUG = "prime-matrix-phi-lpf-latest-new-joint-alpha-terminal-three-atoms-sync"
OUT_LEDGER = DATA / f"{SLUG}-ledger.json"
OUT_JSON = DOCS / f"{SLUG}-router.json"
OUT_MD = DOCS / f"{SLUG}-router.md"

CURRENT_ALPHA = DOCS / "prime-matrix-phi-lpf-latest-new-joint-trace-exit-source-rank-sync-router.json"
...
def build_rows(
    ...
    """生成 latest new-joint alpha/terminal-three-atoms 同步判定表。"""
    terminal_atoms = set(post_three.get("terminal_atoms", []))
    return [
        row(
            "CurrentNewJointAlphaFrontierImported",
            current.get("next_primary_attack_target") == ALPHA_ROW
            and current.get("pointwise_kernel_frontier_imported") is True,
            False,
            "刚提交的 new-joint trace-exit/source-rank 同步层已把直接主攻推进到 alpha row anchor/phase。",
            ALPHA_ROW,
        ),
        row(
        ...
        row(
            ...
            False,
            False,
            "本层只把当前 new-joint alpha 前沿接到终端三原子；未证明三命题无条件闭合。",
...
def build_certificate() -> dict[str, Any]:
    ...
    cert = {
        "certificate_type": "prime_matrix_phi_lpf_latest_new_joint_alpha_terminal_three_atoms_sync_router",
        "status": "phi_lpf_latest_new_joint_alpha_synced_to_terminal_three_atoms_open",
        ...
        "current_new_joint_alpha_frontier_imported": rows[0]["closed"],
        ...
        "plain_conclusion": (
            f"本步把当前 new-joint latest 的 `{ALPHA_ROW}` 接到已有 alpha-terminal 三原子前沿。"
...
def render_markdown(cert: dict[str, Any]) -> str:
    """渲染 Markdown 证书。"""
    lines = [
        "# Prime Matrix Phi-LPF latest new-joint alpha terminal three-atoms sync 证书",
        "",
        f"**状态：** `{cert['status']}`",
        "",
        ...
        "",
        "```text",
        f"current_new_joint_alpha_frontier_imported={fmt_bool(cert['current_new_joint_alpha_frontier_imported'])}",
