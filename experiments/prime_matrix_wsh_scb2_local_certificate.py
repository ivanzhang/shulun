#!/usr/bin/env python3
"""生成 WSH-Hall 的 SCB-2 短块局部证书。

用法示例：
  python3 experiments/prime_matrix_wsh_scb2_local_certificate.py
  python3 experiments/prime_matrix_wsh_scb2_local_certificate.py --max-p 1000

SCB-2 目标：
  对所有 |B|<=3 的连续平衡双尾半素数短块，审计
  |N_R(B)|-|B|，并记录尾标签、固定偏移、小轮相位和端点镜像压力。

注意：这是有限局部证书，不是全局 SCB-2 证明。
"""
...
def is_connected_block(block_semis: list[int], radius: int) -> bool:
    """判断短块的半径邻域是否连通。"""
    return all(
        block_semis[index + 1] - block_semis[index] <= 2 * radius + 1
        for index in range(len(block_semis) - 1)
    )


def classify_pressure(block: dict) -> list[str]:
    """给短块分配有限审计中的压力标签。"""
    ...
    if pressure["max_allowed_offset_load"] >= max(2, size):
    ...
    if not block["connected"]:
        labels.append("Disconnected-additive")
    return labels


def audit_short_blocks(args: argparse.Namespace) -> dict:
    """审计全部 |B|<=3 的短块。"""
    ...
    block_count_by_size: Counter[int] = Counter()
    tight_count_by_signature: Counter[tuple[int, int]] = Counter()
    pressure_label_counter: Counter[str] = Counter()
    connected_counter: Counter[str] = Counter()
    min_surplus_by_size: dict[int, int | None] = {1: None, 2: None, 3: None}
    negative_blocks = 0
    zero_blocks = 0
        ...
        for row in range(1, q + 1):
            ...
            for start_index in range(len(semiprimes)):
                ...
                for end_index in range(
                    start_index,
                    min(len(semiprimes), start_index + args.max_block_size),
                ):
                    ...
                    size = len(block_semis)
                    prime_count = count_in_union(row_primes, merged)
                    surplus = prime_count - size
                    block_count_by_size[size] += 1
                    current_min = min_surplus_by_size[size]
                    if current_min is None or surplus < current_min:
                        min_surplus_by_size[size] = surplus
                    if surplus < 0:
                        negative_blocks += 1
                    if surplus == 0:
                        zero_blocks += 1
                    connected = is_connected_block(block_semis, radius)
                    connected_counter["connected" if connected else "disconnected"] += 1
    ...
    return {
        ...
        "summary": {
            ...
            "block_count_by_size": dict(sorted(block_count_by_size.items())),
            "min_surplus_by_size": {
                str(size): value for size, value in sorted(min_surplus_by_size.items())
            },
            ...
            "pressure_label_counts": dict(sorted(pressure_label_counter.items())),
            "connected_counts": dict(sorted(connected_counter.items())),
        ...
        "tight_blocks": tight_blocks[: args.max_records],
    }
...
def write_markdown(result: dict, path: Path) -> None:
    ...
    lines = [
        ...
        f"- `max_block_size`: `{params['max_block_size']}`",
        ...
        f"- 按块长计数：`{summary['block_count_by_size']}`。",
        f"- 按块长最小 Hall 余量：`{summary['min_surplus_by_size']}`。",
        f"- 负余量块数：`{summary['negative_blocks']}`。",
        f"- 零余量块数：`{summary['zero_blocks']}`。",
        f"- 小余量块数：`{summary['tight_block_count']}`。",
        f"- 小余量签名：`{summary['tight_count_by_signature']}`。",
        f"- 压力标签计数：`{summary['pressure_label_counts']}`。",
        f"- 连通性计数：`{summary['connected_counts']}`。",
        f"- 小余量最大尾标签负载：`{summary['max_tail_load_in_tight_blocks']}`。",
        f"- 小余量最大固定允许偏移负载：`{summary['max_allowed_offset_load_in_tight_blocks']}`。",
        "",
        "## 最紧短块样本",
        "",
    ...
    for block in result["tight_blocks"][:50]:
        lines.append(
            "| {p} | {q} | {row} | {radius} | {size} | {primes} | {surplus} | {connected} | {labels} | {semis} | {mirror} |".format(
                p=block["p"],
                q=block["q"],
                row=block["row"],
                radius=block["radius"],
...
def main() -> None:
    ...
    parser.add_argument("--tight-surplus", type=int, default=2)
    parser.add_argument("--max-block-size", type=int, default=3)
    parser.add_argument("--wheel-primes", default="2,3,5,7,11,13")
    parser.add_argument("--max-records", type=int, default=200)
    parser.add_argument(
        "--out-prefix",
        default="docs/monograph/prime-matrix-wsh-scb2-local-certificate",
    )
    args = parser.parse_args()
    result = audit_short_blocks(args)
