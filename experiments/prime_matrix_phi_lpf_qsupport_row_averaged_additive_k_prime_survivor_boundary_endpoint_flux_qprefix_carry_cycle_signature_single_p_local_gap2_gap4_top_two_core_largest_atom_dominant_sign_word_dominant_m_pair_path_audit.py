#!/usr/bin/env python3
"""审计 dominant sign word 中 dominant m-pair 的端点路径坐标分解。

用法示例：
  python3 experiments/prime_matrix_phi_lpf_qsupport_row_averaged_additive_k_prime_survivor_boundary_endpoint_flux_qprefix_carry_cycle_signature_single_p_local_gap2_gap4_top_two_core_largest_atom_dominant_sign_word_dominant_m_pair_path_audit.py
  python3 -m json.tool docs/monograph/prime-matrix-phi-lpf-qsupport-row-averaged-additive-k-prime-survivor-boundary-endpoint-flux-qprefix-carry-cycle-signature-single-p-local-gap2-gap4-top-two-core-largest-atom-dominant-sign-word-dominant-m-pair-path-audit.json

上一层把 dominant sign word `--+-+` 的 30 质量拆成 3 个路径见证。
其中 dominant m-pair `[769, 773]` 的质量为 20。本层只处理这个 20
质量块，并把它拆成两个端点坐标路径见证。该层关闭有限坐标账本，
...
SLUG = (
    ...
    "gap2-gap4-top-two-core-largest-atom-dominant-sign-word-dominant-m-pair-path"
)
...
def endpoint_orientation(P: int, m_pair: list[int]) -> str:
    """判定 m-pair 相对 P 的位置。"""
    if all(m > P for m in m_pair):
        return "above_P"
    if all(m < P for m in m_pair):
        return "below_P"
    if any(m == P for m in m_pair):
        return "touches_P"
    return "straddles_P"
...
def load_previous_payload() -> dict[str, Any]:
    """读入上一层 path 审计证书。"""
    return json.loads(PREVIOUS_PATH_AUDIT.read_text())


def selected_dominant_m_pair_rows(previous_payload: dict[str, Any]) -> list[dict[str, Any]]:
    """抽取 dominant m-pair 的路径见证行。"""
    rows = previous_payload["finite_audit"]["path_witness_rows"]
    return [row for row in rows if row["m_pair"] == TARGET_M_PAIR]


def build_coordinate_row(row: dict[str, Any]) -> dict[str, Any]:
    """把上一层路径见证行扩展为端点坐标 witness。"""
    P = int(row["P"])
    ...
    offsets = [m - P for m in TARGET_M_PAIR_LIST]
    orientation = endpoint_orientation(P, TARGET_M_PAIR_LIST)
    return {
        "witness_id": f"P{P}_packet{row['packet_index']}_q{row['q_prefix_count']}_mpair_769_773",
        "P": P,
        "P_band": row["P_band"],
        "packet_index": row["packet_index"],
        "edge_mass": row["edge_mass"],
        "m_pair": row["m_pair"],
        "m_left": TARGET_M_PAIR_LIST[0],
        "m_right": TARGET_M_PAIR_LIST[1],
...
def audit(max_prime: int = 1009) -> dict[str, Any]:
    """审计 dominant m-pair endpoint coordinate path 账本。"""
    previous_payload = load_previous_payload()
    previous = previous_payload["finite_audit"]
    rows = selected_dominant_m_pair_rows(previous_payload)
    coordinate_rows = [build_coordinate_row(row) for row in rows if row["P"] <= max_prime]
    ...
    all_q_prefix_le_10 = all(row["q_prefix_band"] == "q<=10" for row in coordinate_rows)
    ...
    ledger_closed = (
        previous_payload["dominant_sign_word_path_ledger_closed"]
        and previous["dominant_m_pair"] == TARGET_M_PAIR
        and previous["dominant_m_pair_edge_mass"] == 20
        and previous["q_prefix_band_q_le_10_edge_mass"] == 20
        and total_witness_count == 2
        and total_edge_mass == 20
        and all_edge_mass_ten
        and all_gap4
        and all_occurrence_count_two
        and all_q_prefix_le_10
        ...
        and len(raw_base_edge_mass) == 2
        and len(signed_child_edge_mass) == 2
        and orientation_edge_mass["above_P"] == 10
        and orientation_edge_mass["below_P"] == 10
        and set(p_edge_mass) == {607, 953}
        and set(q_prefix_count_edge_mass) == {7, 10}
        and set(m_shell_prime_count_edge_mass) == {3, 4}
    )

    return {
        "max_prime": max_prime,
        "previous_dominant_sign_word_path_ledger_closed": previous_payload[
            "dominant_sign_word_path_ledger_closed"
        ],
        "target_sign_word": TARGET_SIGN_WORD,
        "target_m_pair": TARGET_M_PAIR,
        "previous_dominant_m_pair_edge_mass": previous["dominant_m_pair_edge_mass"],
        "dominant_m_pair_endpoint_coordinate_path_ledger_closed": ledger_closed,
        "endpoint_coordinate_witness_count": total_witness_count,
        "endpoint_coordinate_edge_mass": total_edge_mass,
        "all_endpoint_edge_mass_equals_10": all_edge_mass_ten,
        "all_endpoint_integer_gap_equals_4": all_gap4,
        "all_endpoint_occurrence_count_equals_2": all_occurrence_count_two,
        "all_endpoint_q_prefix_band_q_le_10": all_q_prefix_le_10,
        "all_endpoint_m_shell_band_m_le_4": all_m_shell_le_4,
        "all_endpoint_cycle_length_equals_5": all_cycle_length_five,
        "all_endpoint_sign_word_is_target": all_sign_word_target,
        "all_endpoint_sign_switch_count_equals_3": all_switch_count_three,
        "all_endpoint_sign_balance_plus2_minus3": all_sign_balance_fixed,
        ...
        "P_edge_rows": edge_rows(p_edge_mass, total_edge_mass, "P"),
...
def build_payload() -> dict[str, Any]:
    ...
    finite_audit = audit()
    return {
        "certificate_type": "prime_matrix_phi_lpf_qsupport_row_averaged_additive_k_prime_survivor_boundary_endpoint_flux_qprefix_carry_cycle_signature_single_p_local_gap2_gap4_top_two_core_largest_atom_dominant_sign_word_dominant_m_pair_path_audit",
        "frontier_verified_date": FRONTIER_VERIFIED_DATE,
        "status": "dominant_m_pair_endpoint_coordinate_path_ledger_closed_family_bound_open",
        "chosen_claim": "Prime Matrix row/column Phi-LPF",
        "reason_chosen": "the dominant m-pair [769,773] is the largest remaining block inside the dominant sign-word path ledger",
        "current_object": {
        ...
        "closed_gates": [
            ...
            gate(
                "DominantMPairEndpointCoordinatePathLedger",
                finite_audit["dominant_m_pair_endpoint_coordinate_path_ledger_closed"],
                finite_audit["dominant_m_pair_endpoint_coordinate_path_ledger_closed"],
                "The [769,773] mass-20 block is split into two endpoint coordinate witnesses.",
                "none for the finite coordinate ledger",
            ),
            gate(
                "DominantMPairEndpointCoordinateUniformFamilyBound",
                False,
                False,
                "Control the two coordinate witnesses as a uniform family.",
                "finite coordinate audit gives exact witnesses but no global theorem",
            ),
            gate(
                "OtherDominantSignWordMPairBound",
                False,
                False,
                "Control the non-dominant m-pair [757,761] witness in the sign word.",
                "carried forward from the 3-path ledger",
            ),
            gate(
        ...
        "dominant_m_pair_endpoint_coordinate_path_ledger_closed": finite_audit[
...
def build_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"dominant_m_pair_endpoint_coordinate_path_ledger_closed={str(audit_result['dominant_m_pair_endpoint_coordinate_path_ledger_closed']).lower()}",
        f"target_sign_word={audit_result['target_sign_word']}",
        f"target_m_pair={audit_result['target_m_pair']}",
        f"previous_dominant_m_pair_edge_mass={audit_result['previous_dominant_m_pair_edge_mass']}",
        f"endpoint_coordinate_witness_count={audit_result['endpoint_coordinate_witness_count']}",
        f"endpoint_coordinate_edge_mass={audit_result['endpoint_coordinate_edge_mass']}",
        f"all_endpoint_edge_mass_equals_10={str(audit_result['all_endpoint_edge_mass_equals_10']).lower()}",
        f"all_endpoint_integer_gap_equals_4={str(audit_result['all_endpoint_integer_gap_equals_4']).lower()}",
        f"all_endpoint_occurrence_count_equals_2={str(audit_result['all_endpoint_occurrence_count_equals_2']).lower()}",
        f"all_endpoint_q_prefix_band_q_le_10={str(audit_result['all_endpoint_q_prefix_band_q_le_10']).lower()}",
        f"all_endpoint_m_shell_band_m_le_4={str(audit_result['all_endpoint_m_shell_band_m_le_4']).lower()}",
        f"all_endpoint_cycle_length_equals_5={str(audit_result['all_endpoint_cycle_length_equals_5']).lower()}",
        f"all_endpoint_sign_word_is_target={str(audit_result['all_endpoint_sign_word_is_target']).lower()}",
        f"all_endpoint_sign_switch_count_equals_3={str(audit_result['all_endpoint_sign_switch_count_equals_3']).lower()}",
        f"all_endpoint_sign_balance_plus2_minus3={str(audit_result['all_endpoint_sign_balance_plus2_minus3']).lower()}",
        f"distinct_raw_base_template_count={audit_result['distinct_raw_base_template_count']}",
        f"distinct_signed_child_count={audit_result['distinct_signed_child_count']}",
        ...
        *markdown_table(
        ...
        "",
        *markdown_table(
            audit_result["q_prefix_count_edge_rows"],
        ...
        *payload["latest_narrowest_mouth"],
        ...
        "",
        "```text",
        f"dominant_m_pair_endpoint_coordinate_path_ledger_closed={str(payload['dominant_m_pair_endpoint_coordinate_path_ledger_closed']).lower()}",
        f"dominant_m_pair_endpoint_coordinate_family_bound_proved={str(payload['dominant_m_pair_endpoint_coordinate_family_bound_proved']).lower()}",
...
def main() -> None:
    ...
    print(
        "dominant_m_pair_endpoint_coordinate_path_ledger_closed="
        f"{payload['dominant_m_pair_endpoint_coordinate_path_ledger_closed']}"
    )
    print(f"endpoint_coordinate_edge_mass={audit_result['endpoint_coordinate_edge_mass']}")
    print(f"endpoint_coordinate_witness_count={audit_result['endpoint_coordinate_witness_count']}")
