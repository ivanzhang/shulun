...
该脚本不试图证明主定理，只把当前文档中已经显式暴露的
全局闸门整理为可审查的阻塞清单，防止把条件归约误写成无条件证明。
"""
...
def build_audit() -> dict:
    blocking_items = [
        {
            "id": "B1_UAS_DBA_four_point_energy",
            "area": "行侧短段双线性/FS8-polymer/B.0.4*",
            "status": "local_certificate_chain_reviewed_modulo_final_acceptance",
            "evidence": [
                "docs/final-proof-draft.md:3211",
                "docs/final-proof-draft.md:3693",
                "docs/final-proof-draft.md:4160",
                "docs/final-proof-draft.md:4247",
                "docs/b1-dba-final-acceptance-review.md",
            ],
            "blocking_claim": "行侧 B1 sawtooth 硬路线已压缩为接受既有 DBA-A1--A5、FS8/FNL-KS 与 R1/R2 终审证书；不再是新的局部硬点，但仍需终稿审查接受。",
            "minimal_remaining": [
        ...
        {
            "id": "B2_G1_global_capacity_defect",
            "area": "D4/R5 递归剥离全局容量闸门",
            "status": "not_closed_global_local_audits_available",
            "evidence": [
                "docs/d4-r5-G1-capacity-audit.json",
                "docs/d4-r5-G1-lyapunov-energy-audit.json",
                "docs/d4-r5-G1-capacity-route.md:139",
                "docs/d4-r5-G1-global-transitivity-audit.md",
            ],
            "blocking_claim": "G1 局部容量证据很强，但必须证明递归状态语义不变、backflow 投影可传递、jump/终端窗口族全局覆盖。",
            "minimal_remaining": [
        ...
        {
            "id": "B3_R5_G3_G4_G5_globalization",
            "area": "R5global jump/backflow/base certificate",
            "status": "not_closed",
            "evidence": [
                "docs/d4-r5-global-rpl-gates.json",
                "docs/d4-r5-current-candidate-closure-status.md:25",
            ],
            "blocking_claim": "G3/G4/G5 仍需全局边界穿越计数、backflow 求和、终端窗口族覆盖。",
            "minimal_remaining": [
                "jump 阈值穿越次数/误差求和证明",
        ...
        {
            "id": "B4_column_LC_pointwise_capacity",
            "area": "列命题/非零列点态容量",
            "status": "not_closed_or_conditionally_reduced",
            "evidence": [
                "docs/final-proof-draft.md:996",
                "docs/final-proof-draft.md:12189",
                "docs/final-proof-draft.md:12363",
            ],
            "blocking_claim": "列侧需要点态高阈值筛余容量下界或等价强等差素数/证书输入。",
            "minimal_remaining": [
        ...
        {
            "id": "B5_explicit_threshold_and_finite_verification",
            "area": "显式阈值与有限验证",
            "status": "not_closed",
            "evidence": [
                "docs/final-proof-draft.md:7",
                "docs/final-proof-draft.md:48",
                "docs/d4-r5-review-grade-status.md:28",
            ],
            "blocking_claim": "即便高段证明闭合，仍需抽取 P_* 并完成 P<=P_* 的可复核有限验证。",
            "minimal_remaining": [
                "从所有行列接口抽取显式 P_*",
                "实现有限验证证书格式",
                "运行并归档阈值以下全部奇素数验证",
    ...
    return {
        "certificate_type": "global_unconditional_closure_audit",
        "status": "not_unconditionally_closed",
        "date": "2026-04-29",
        "conclusion": "当前文稿仍是条件化/归约型证明框架。若要升级为全局无条件证明，必须逐项闭合 blocking_items。",
        "closed_or_strong_local_components": [
        ...
        "minimal_path_to_unconditional_closure": [
            "终稿接受 B1 证书链，确保行侧 sawtooth 硬路线表述为条件明确的局部闭合。",
            "优先闭合 B2/B3，把 D4/R5 递归剥离证书全局化。",
            "闭合 B4，完成列侧 LC 点态容量。",
            "抽取显式阈值并完成 B5 有限验证。",
        ],
        "honesty_guardrail": "在所有 blocking_items 均为 closed 且有限验证完成前，不得声称全部奇素数主定理已证明。",
    }
...
def write_markdown(audit: dict) -> None:
    ...
    for item in audit["closed_or_strong_local_components"]:
        lines.append(f"- {item}")
    lines += ["", "## 阻塞项"]
    for block in audit["blocking_items"]:
        lines += [
            f"### {block['id']}：{block['area']}",
            f"- 状态：`{block['status']}`",
            f"- 阻塞命题：{block['blocking_claim']}",
            "- 剩余义务：",
        ]
        for remaining in block["minimal_remaining"]:
            lines.append(f"  - {remaining}")
        lines.append("- 证据位置：" + ", ".join(block["evidence"]))
        lines.append("")
    lines += ["## 最小闭合路径"]
    for index, step in enumerate(audit["minimal_path_to_unconditional_closure"], 1):
    ...
    (DOCS / "global-unconditional-closure-audit.md").write_text("\n".join(lines) + "\n", encoding="utf-8")


def main() -> None:
    audit = build_audit()
    (DOCS / "global-unconditional-closure-audit.json").write_text(
        json.dumps(audit, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    write_markdown(audit)
    print(DOCS / "global-unconditional-closure-audit.json")
    print(DOCS / "global-unconditional-closure-audit.md")
