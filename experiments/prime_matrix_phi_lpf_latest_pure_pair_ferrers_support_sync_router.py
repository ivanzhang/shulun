#!/usr/bin/env python3
"""生成 Phi-LPF 最新 pure-pair atom 到 Ferrers support 的同步证书。

用法示例：
  python3 experiments/prime_matrix_phi_lpf_latest_pure_pair_ferrers_support_sync_router.py
  python3 -m json.tool docs/monograph/prime-matrix-phi-lpf-latest-pure-pair-ferrers-support-sync-router.json
...
SLUG = "prime-matrix-phi-lpf-latest-pure-pair-ferrers-support-sync"
OUT_LEDGER = DATA / f"{SLUG}-ledger.json"
OUT_JSON = DOCS / f"{SLUG}-router.json"
OUT_MD = DOCS / f"{SLUG}-router.md"

LATEST_PURE_CERT = DOCS / "prime-matrix-phi-lpf-latest-offdiagonal-pure-pair-atom-sync-router.json"
    ...
    return [
        {
            ...
            "meaning": "pure pair atom 先剥离 Ferrers 支撑/度数账本，剩余才是每条边的 signed kernel。",
        },
        {
        ...
        },
        {
            ...
            "meaning": "最新 pure-pair atom 被收窄为 two-prime signed interaction kernel 与配套字段。",
        },
    ]
...
def build_rows(
    ...
    """生成最新 Ferrers support 同步判定表。"""
    atoms = source_atoms()
    return [
        row(
            "LatestPurePairAtomImported",
            latest.get("next_primary_attack_target") == PURE_SEMIPRIME_ATOM,
            False,
            "上一层最新 pure-pair sync 已把直接硬点定位到 pure semiprime pair signed atom。",
            PURE_SEMIPRIME_ATOM,
        ),
        row(
            ...
            True,
            "既有 Ferrers support 证书可直接作用在最新 pure-pair atom 入口。",
            FERRERS_SUPPORT,
        ),
        row(
        ...
        row(
            ...
            False,
            "最新 pure-pair atom 被收窄为 two-prime signed interaction kernel 与配套字段。",
            TWO_PRIME_KERNEL,
        ),
        row(
        ...
        row(
            ...
            False,
            False,
            "本步只同步 Ferrers support 剥离；未证明三命题无条件闭合。",
            f"{TWO_PRIME_KERNEL} AND {OFFDIAG_ORIENTATION} AND {OFFDIAG_EXACTUV} AND {INTERNAL_TRANSITION} AND {atoms}",
        ),
    ]


def build_certificate() -> dict[str, Any]:
    """组装最新 Ferrers support 同步证书。"""
    latest = load_json(LATEST_PURE_CERT)
    ...
    return {
        "certificate_type": "prime_matrix_phi_lpf_latest_pure_pair_ferrers_support_sync_router",
        "status": "phi_lpf_latest_pure_pair_atom_synced_to_ferrers_support_open",
        "same_theorem_target_preserved": True,
        "no_theorem_switch": True,
        "frontier_sync_only": True,
        "finite_evidence_not_used_as_global_proof": True,
        "latest_pure_pair_atom_imported": rows[0]["closed"],
        "ferrers_support_router_imported": rows[1]["closed"],
        "ferrers_support_rule_synced": rows[2]["closed"],
        "ferrers_degree_ledger_synced": rows[3]["closed"],
        "support_graph_signed_kernel_emission_proved": False,
        "source_atoms_carried_forward": rows[5]["closed"],
        "latest_basis_replaces_pure_atom_with_two_prime_kernel": rows[6]["closed"],
        ...
        "plain_conclusion": (
            "本步把最新 `PhiLPFOffDiagonalPureSemiprimePairSignedSeedAtomBeforePushforward` "
            ...
            "internal transition 与 source 三原子。"
        ),
    }
...
def render_markdown(cert: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "```text",
        f"latest_pure_pair_atom_imported={fmt_bool(cert['latest_pure_pair_atom_imported'])}",
