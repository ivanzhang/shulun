#!/usr/bin/env python3
"""生成 Phi-LPF 最新 edge multiplier 到 first-edge slab 的同步证书。

用法示例：
  python3 experiments/prime_matrix_phi_lpf_latest_edge_multiplier_slab_sync_router.py
  python3 -m json.tool docs/monograph/prime-matrix-phi-lpf-latest-edge-multiplier-slab-sync-router.json
...
SLUG = "prime-matrix-phi-lpf-latest-edge-multiplier-slab-sync"
OUT_LEDGER = DATA / f"{SLUG}-ledger.json"
OUT_JSON = DOCS / f"{SLUG}-router.json"
OUT_MD = DOCS / f"{SLUG}-router.md"

LATEST_TRANSPORT_CERT = DOCS / "prime-matrix-phi-lpf-latest-bucket-transport-stack-sync-router.json"
    ...
    return [
        {
            ...
            "meaning": "逐 edge signed multiplier 表按 `prefix=1` 第一边与 `prefix>1` 内部边唯一拆分。",
        },
        {
        ...
        },
        {
            ...
            "meaning": "semiprime first seed 仍需推前前 signed payload，不能由 Phi fiber 反推。",
        },
        {
            "from": INTERNAL_TRANSITION,
            "to": "nonzero predecessor / signed local factor / path product compatibility",
            "meaning": "内部 prime-adjoin transition 仍需非零前缀或命名回流，不能用未知终值后验除法。",
        },
    ]


def build_rows(data: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    """生成最新 edge multiplier slab 同步判定表。"""
    latest = data["latest"]
    ...
    return [
        row(
            "LatestTransportHardpointImported",
            latest.get("next_primary_attack_target") == EDGE_MULTIPLIER,
            False,
            "上一层最新 transport-stack 已把 bucket signed law 压到逐 edge signed multiplier 表。",
            EDGE_MULTIPLIER,
        ),
        row(
            ...
            True,
            "既有 first-edge slab 证书可作为最新 edge multiplier hardpoint 的直接下游。",
            f"{FIRST_SEED} AND {INTERNAL_TRANSITION}",
        ),
        row(
            "EdgeMultiplierSplitSyncedToLatestBasis",
            latest.get("next_primary_attack_target") == EDGE_MULTIPLIER
            and first_edge.get("edge_table_split_into_first_seed_and_internal_transition") is True,
            True,
            "最新保留基中的 edge multiplier 可替换为 first seed slab 与 internal transition 的合取。",
            f"{FIRST_SEED} AND {INTERNAL_TRANSITION}",
        ),
        row(
        ...
        row(
            ...
            False,
            "first seed 的下一层已可继续拆为 diagonal common packet 与 offdiagonal seed，但这不闭合 signed 表。",
            semiprime.get("retained_basis_after_router", FIRST_SEED),
        ),
        row(
            "BaseSourcePacketAtomsCarriedForward",
            latest.get("parallel_primary_attack_target") == f"{ALPHA_ANCHOR} AND {ARITH_ID} AND {RANK_CERT}",
            False,
            "edge slab 只替换 edge multiplier；base/source-packet 三原子仍保留在最新合取基中。",
        ...
        row(
            ...
            False,
            False,
            "本步只重接最新 hardpoint 与既有 slab 拆解；未证明 first seed、internal transition、source 三原子、ExactUV、模型、Rate 或 DStructure。",
            f"{FIRST_SEED} AND {INTERNAL_TRANSITION}",
        ),
    ]
...
def build_certificate() -> dict[str, Any]:
    """组装最新 edge multiplier slab 同步证书。"""
    data = {
        "latest": load_json(LATEST_TRANSPORT_CERT),
    ...
    latest_basis = (
        f"(({ALPHA_ANCHOR} AND {ARITH_ID} AND {RANK_CERT} AND {FIRST_SEED} AND {INTERNAL_TRANSITION}) "
        f"OR {POINTWISE_TABLE} OR {BRANCH_TRACE} OR {ATOMIC_TRACE} OR {SEED_CYCLE_CUT} OR "
        f"{TERMINAL_DESCENT} OR {PDEC_SCOPE} OR {NEW_JOINT}) "
        f"AND {EXACTUV_PAIR} AND {MODEL} AND {RATE} AND {DSTRUCTURE}"
    )
    return {
        "certificate_type": "prime_matrix_phi_lpf_latest_edge_multiplier_slab_sync_router",
        "status": "phi_lpf_latest_edge_multiplier_synced_to_first_edge_slab_open",
        "same_theorem_target_preserved": True,
        "no_theorem_switch": True,
        "frontier_sync_only": True,
        "finite_evidence_not_used_as_global_proof": True,
        "latest_transport_hardpoint_imported": rows[0]["closed"],
        "first_edge_slab_router_imported": rows[1]["closed"],
        "edge_multiplier_split_synced_to_latest_basis": rows[2]["closed"],
        "first_edge_phi_fiber_formula_imported": rows[3]["closed"],
        "phi_fiber_unsigned_only_guard": rows[4]["closed"],
        ...
        "plain_conclusion": (
            "本步把最新 `PhiLPFRoughCofactorStepSignedMultiplierTableBeforePushforward` "
            "接入既有 first-edge slab 证书。edge multiplier 表不是最终黑箱；"
            "它按 LPF ordered path 唯一拆成 semiprime first-edge signed seed table 与 "
            "internal prime-adjoin signed transition law。Phi fiber 公式只支付第一边 `(p,q)` "
            "的 q-rough continuation occurrence mass，不产生 signed seed、local factor、orientation "
            "或 ExactUV payload。因此最新 edge 侧硬点是 first seed 与 internal transition 的合取，"
            "同时上一层 base/source-packet 三原子仍保留。行/列命题仍未无条件证明。"
        ),
    }
...
def render_markdown(cert: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "```text",
        f"latest_transport_hardpoint_imported={fmt_bool(cert['latest_transport_hardpoint_imported'])}",
        f"first_edge_slab_router_imported={fmt_bool(cert['first_edge_slab_router_imported'])}",
        f"edge_multiplier_split_synced_to_latest_basis={fmt_bool(cert['edge_multiplier_split_synced_to_latest_basis'])}",
