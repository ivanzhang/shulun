#!/usr/bin/env python3
"""生成 constructor-latest new-payload 到 source-atom alignment 的同步证书。

用法示例：
  python3 experiments/prime_matrix_phi_lpf_latest_constructor_new_payload_source_atom_alignment_sync_router.py
  python3 -m json.tool docs/monograph/prime-matrix-phi-lpf-latest-constructor-new-payload-source-atom-alignment-sync-router.json
...
SLUG = "prime-matrix-phi-lpf-latest-constructor-new-payload-source-atom-alignment-sync"
OUT_LEDGER = DATA / f"{SLUG}-ledger.json"
OUT_JSON = DOCS / f"{SLUG}-router.json"
OUT_MD = DOCS / f"{SLUG}-router.md"

LATEST_CONSTRUCTOR_TRACE_SYNC_CERT = (
    DOCS / "prime-matrix-phi-lpf-latest-constructor-signed-atom-trace-sync-router.json"
)
...
def source_atom_contract() -> list[dict[str, str]]:
    """列出 constructor latest new-payload 若要破环必须携带的 source 原子字段。"""
    atoms = source_atoms()
    return [
        {
            "field": "same_trace_key_lock",
            "requirement": "继承 constructor latest signed atom trace-sync 的同一 pre-Cauchy trace key。",
            "remaining": NEW_PAYLOAD,
        },
        {
            "field": "constructor_source_three_atoms",
            "requirement": "constructor 分支继续携带 alpha anchor、arithmetic identity 与 ExactUV rank multiplicity 三原子。",
            "remaining": atoms,
        },
        {
            "field": "same_unit_row_mass_and_signed_survival",
            "requirement": "payload 不能替代 row-mass/no-heavy-row 或 signed survival。",
...
def largest_slot_sample(latest_trace: dict[str, Any]) -> dict[str, Any]:
    """抽取 constructor latest trace-sync 的最大 slot 样本。"""
...
def build_rows(
    ...
    """生成 constructor latest new-payload/source-atom 同步判定表。"""
    ...
    return [
        row(
            "LatestConstructorNewPayloadImported",
            latest_trace.get("next_primary_attack_target") == NEW_PAYLOAD,
            False,
            "上一层 constructor trace-sync 已把生产性硬点定位到 new primitive payload/trace。",
            NEW_PAYLOAD,
        ),
        row(
            "ConstructorSourceAndSideGatesCarried",
            atoms in latest_trace.get("paired_constructor_required_targets", [])
            and latest_trace.get("parallel_constructor_side_gates") == f"{NONZERO_SURVIVAL} AND {ROW_MASS}",
            False,
        ...
        row(
            "LatestConstructorNewPayloadReducedToSourceRankAtom",
            source_rank_imported
        ...
        row(
            ...
            latest_trace.get("constructor_source_atoms_carried_forward") is True,
            False,
        ...
        row(
            ...
            latest_trace.get("nonzero_signed_row_survival_proved") is False
            and latest_trace.get("same_formal_unit_row_mass_normalization_proved") is False,
            False,
        ...
        row(
            ...
            (
                ...
                f"AND {COMPLETE_KEY} AND {FIXED_KEY_MULT}"
            ),
        ),
    ]
...
def build_certificate() -> dict[str, Any]:
    """组装 constructor latest new-payload/source-atom alignment 同步证书。"""
    latest_trace = load_json(LATEST_CONSTRUCTOR_TRACE_SYNC_CERT)
    ...
    return {
        "certificate_type": "prime_matrix_phi_lpf_latest_constructor_new_payload_source_atom_alignment_sync_router",
        "status": "phi_lpf_latest_constructor_new_payload_reduced_to_source_atom_package_open",
        "same_theorem_target_preserved": True,
        "no_theorem_switch": True,
        "frontier_sync_only": True,
        "finite_evidence_not_used_as_global_proof": True,
        "latest_constructor_new_payload_imported": rows[0]["closed"],
        "constructor_source_and_side_gates_carried": rows[1]["closed"],
        "latest_unsigned_labels_cannot_pay_payload": rows[2]["closed"],
        "signed_lane_cycle_cut_carried": rows[3]["closed"],
        "strict_new_payload_alignment_imported": rows[4]["closed"],
        "latest_constructor_new_payload_reduced_to_source_rank_atom": rows[5]["closed"],
        "constructor_source_atoms_carried_forward": rows[7]["closed"],
        "constructor_row_mass_and_survival_still_open": rows[8]["closed"],
        ...
        "plain_conclusion": (
            "本步把 constructor latest `NewPrimitiveAtomicSignedPayloadOrTraceFormulaArtifact` "
            ...
            "constructor source 三原子、row-mass/no-heavy-row 与 signed survival 继续作为配套义务；"
...
def render_markdown(cert: dict[str, Any]) -> str:
    """渲染 Markdown 证书。"""
    lines = [
        "# Prime Matrix Phi-LPF latest constructor new-payload source-atom alignment sync 证书",
        "",
        f"**状态：** `{cert['status']}`",
        "",
        ...
        "",
        "```text",
        f"latest_constructor_new_payload_imported={fmt_bool(cert['latest_constructor_new_payload_imported'])}",
        f"constructor_source_and_side_gates_carried={fmt_bool(cert['constructor_source_and_side_gates_carried'])}",
        ...
        f"latest_constructor_new_payload_reduced_to_source_rank_atom={fmt_bool(cert['latest_constructor_new_payload_reduced_to_source_rank_atom'])}",
        f"constructor_source_atoms_carried_forward={fmt_bool(cert['constructor_source_atoms_carried_forward'])}",
        f"constructor_row_mass_and_survival_still_open={fmt_bool(cert['constructor_row_mass_and_survival_still_open'])}",
