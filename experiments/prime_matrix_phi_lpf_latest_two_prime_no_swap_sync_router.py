#!/usr/bin/env python3
"""生成 Phi-LPF 最新 two-prime signed kernel 到 no-swap 的同步证书。

用法示例：
  python3 experiments/prime_matrix_phi_lpf_latest_two_prime_no_swap_sync_router.py
  python3 -m json.tool docs/monograph/prime-matrix-phi-lpf-latest-two-prime-no-swap-sync-router.json
...
SLUG = "prime-matrix-phi-lpf-latest-two-prime-no-swap-sync"
OUT_LEDGER = DATA / f"{SLUG}-ledger.json"
OUT_JSON = DOCS / f"{SLUG}-router.json"
OUT_MD = DOCS / f"{SLUG}-router.md"

LATEST_FERRERS_CERT = DOCS / "prime-matrix-phi-lpf-latest-pure-pair-ferrers-support-sync-router.json"
    ...
    return [
        {
            ...
            "meaning": "two-prime kernel 先剥离 LPF owner canonical order；交换对称不能供应 signed kernel。",
        },
        {
        ...
        },
        {
            ...
            "meaning": "最新 two-prime kernel 被收窄为 canonical edge-local signed interaction formula 或命名 return。",
        },
    ]
...
def build_rows(
    ...
    return [
        row(
            "LatestTwoPrimeKernelImported",
            latest.get("next_primary_attack_target") == TWO_PRIME_KERNEL,
            False,
            "上一层最新 Ferrers sync 已把直接硬点定位到 two-prime signed interaction kernel。",
            TWO_PRIME_KERNEL,
        ),
        row(
            ...
            True,
            "既有 ordered no-swap 证书可直接作用在最新 two-prime kernel 入口。",
            ORDERED_NO_SWAP,
        ),
        row(
        ...
        row(
            ...
            False,
            "最新 two-prime kernel 被收窄为 canonical edge-local signed formula 或命名 return。",
            EDGE_LOCAL_FORMULA,
        ),
        row(
        ...
        row(
            ...
            False,
            False,
            "本步只同步 ordered no-swap 剥离；未证明三命题无条件闭合。",
            f"{EDGE_LOCAL_FORMULA} AND {OFFDIAG_ORIENTATION} AND {OFFDIAG_EXACTUV} AND {INTERNAL_TRANSITION} AND {atoms}",
        ),
    ]
...
def build_certificate() -> dict[str, Any]:
    """组装最新 no-swap 同步证书。"""
    latest = load_json(LATEST_FERRERS_CERT)
    ...
    return {
        "certificate_type": "prime_matrix_phi_lpf_latest_two_prime_no_swap_sync_router",
        "status": "phi_lpf_latest_two_prime_kernel_synced_to_ordered_no_swap_open",
        "same_theorem_target_preserved": True,
        "no_theorem_switch": True,
        "frontier_sync_only": True,
        "finite_evidence_not_used_as_global_proof": True,
        "latest_two_prime_kernel_imported": rows[0]["closed"],
        "no_swap_router_imported": rows[1]["closed"],
        "lpf_owner_ordered_no_swap_synced": rows[2]["closed"],
        "product_symmetry_signed_emission_proved": False,
        "source_atoms_carried_forward": rows[4]["closed"],
        "latest_basis_replaces_two_prime_kernel_with_edge_local_formula": rows[5]["closed"],
        ...
        "plain_conclusion": (
            ...
            "formula 或命名 return，并仍需 orientation、ExactUV return、internal transition 与 source 三原子。"
        ),
    }
...
def render_markdown(cert: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "```text",
        f"latest_two_prime_kernel_imported={fmt_bool(cert['latest_two_prime_kernel_imported'])}",
