#!/usr/bin/env python3
"""生成 Phi-LPF latest constructor source-entropy payload-loop cut 同步证书。

用法示例：
  python3 experiments/prime_matrix_phi_lpf_latest_constructor_source_entropy_payload_loop_cut_sync_router.py
  python3 -m json.tool docs/monograph/prime-matrix-phi-lpf-latest-constructor-source-entropy-payload-loop-cut-sync-router.json
...
SLUG = "prime-matrix-phi-lpf-latest-constructor-source-entropy-payload-loop-cut-sync"
OUT_LEDGER = DATA / f"{SLUG}-ledger.json"
OUT_JSON = DOCS / f"{SLUG}-router.json"
OUT_MD = DOCS / f"{SLUG}-router.md"

CONSTRUCTOR_SOURCE_ATOM_CERT = (
...
def build_rows(
    constructor_source: dict[str, Any],
    ...
    """生成 constructor source-entropy payload-loop cut 判定表。"""
    constructor_entropy_imported = (
        constructor_source.get("next_primary_attack_target") == DOMAIN_ENTROPY
        and constructor_source.get("actual_source_domain_entropy_proved") is False
    )
    ...
    return [
        row(
            "LatestConstructorSourceEntropyImported",
            constructor_entropy_imported,
            False,
            "上一层 constructor new-payload/source-atom 已把第一硬点压到 actual source-domain entropy。",
            DOMAIN_ENTROPY,
        ),
        row(
            ...
            True,
            "source entropy 原子化只给出 signed row-mass entropy 包，不证明 signed law、row-mass 或 row support。",
        ...
        row(
            ...
            False,
            False,
            "本步只切掉 constructor payload/source-entropy 自证环；未证明三命题无条件闭合。",
...
def build_certificate() -> dict[str, Any]:
    """组装 constructor source-entropy payload-loop cut 同步证书。"""
    constructor_source = load_json(CONSTRUCTOR_SOURCE_ATOM_CERT)
    ...
    return {
        "certificate_type": "prime_matrix_phi_lpf_latest_constructor_source_entropy_payload_loop_cut_sync_router",
        "status": "phi_lpf_latest_constructor_source_entropy_payload_loop_cut_to_fresh_joint_declaration_open",
        "same_theorem_target_preserved": True,
        "no_theorem_switch": True,
        "frontier_sync_only": True,
        "finite_evidence_not_used_as_global_proof": True,
        "latest_constructor_source_entropy_imported": rows[0]["closed"],
        "source_entropy_atom_boundary_carried": rows[1]["closed"],
        "phi_lpf_candidate_capacity_still_unsigned": rows[2]["closed"],
        "strict_source_entropy_downstream_to_cycle_or_terminal": rows[3]["closed"],
        "cycle_or_terminal_unified_to_joint_declaration": rows[4]["closed"],
        "constructor_upstream_joint_to_payload_chain_imported": rows[5]["closed"],
        "constructor_payload_source_entropy_loop_detected": rows[6]["closed"],
        ...
        "plain_conclusion": (
            ...
            "沿 strict source-entropy downstream 与 cycle-cut/terminal/PDEC 统一前沿展开。"
            "展开后回到 `PreCauchyJointWordCoefficientEmitterDeclarationLineForActualNoncanonicalSourceTuple`；"
            "而该 joint declaration 正是 constructor 上游已经用于推出 built-in pairing、new payload "
            "和 source entropy 的入口。因此当前 constructor 内部路线形成 "
            "`joint declaration -> built-in pairing -> new payload -> source entropy -> cycle/terminal -> joint declaration` "
            "自证环，不能作为闭合证明。非循环推进必须提交不经该 payload 回环的新鲜独立 joint declaration，"
            "或走 canonical-lock、independent source bridge、PDEC/外部谱、逐点 signed table、ExactUV 与 "
            "complete/fixed-key 等开放出口。行/列命题仍未无条件闭合。"
        ),
    }
...
def render_markdown(cert: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "```text",
        f"latest_constructor_source_entropy_imported={fmt_bool(cert['latest_constructor_source_entropy_imported'])}",
        f"source_entropy_atom_boundary_carried={fmt_bool(cert['source_entropy_atom_boundary_carried'])}",
        f"phi_lpf_candidate_capacity_still_unsigned={fmt_bool(cert['phi_lpf_candidate_capacity_still_unsigned'])}",
        f"strict_source_entropy_downstream_to_cycle_or_terminal={fmt_bool(cert['strict_source_entropy_downstream_to_cycle_or_terminal'])}",
        f"cycle_or_terminal_unified_to_joint_declaration={fmt_bool(cert['cycle_or_terminal_unified_to_joint_declaration'])}",
        f"constructor_upstream_joint_to_payload_chain_imported={fmt_bool(cert['constructor_upstream_joint_to_payload_chain_imported'])}",
        f"constructor_payload_source_entropy_loop_detected={fmt_bool(cert['constructor_payload_source_entropy_loop_detected'])}",
        f"raw_constructor_payload_loop_counts_as_closure={fmt_bool(cert['raw_constructor_payload_loop_counts_as_closure'])}",
        f"fresh_joint_declaration_outside_loop_proved={fmt_bool(cert['fresh_joint_declaration_outside_loop_proved'])}",
    ...
    for item in cert["constructor_payload_loop_edges"]:
        lines.append(
    ...
    lines.extend(
        [
            ...
            "",
            "```text",
            cert["productive_joint_atom_underlying_target"],
            "```",
            "",
            "完整 joint 字段基：",
            "",
            "```text",
            cert["fresh_joint_field_basis"],
            "```",
            "",
            "## 4. 最新保留基",
            "",
