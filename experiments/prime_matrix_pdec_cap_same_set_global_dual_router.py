...
def build_rows(
    ...
    diffuse_terminal_split_closed = (
        ...
        == "pdec_cap_diffuse_terminal_split_reduced_to_dense_old_hole_kernel_or_sc9"
    ...
    return [
        ...
        row(
            ...
            "不持久 Gamma 分支剩余自足义务：稠密旧洞选择核触发容量/PDEC/ColumnCRT，或 KL 平坦 clean 残余的 SC-9 谱大筛原子。",
            True,
        ),
        row(
...
def run(
    ...
    return {
        ...
        "frontier_law": (
            ...
            "further to DenseOldHoleKernel/Capacity/PDEC/ColumnCRT or the self-contained "
        ...
        "review_conclusion": (
            ...
            "diffuse 分支又被压到 DenseOldHoleKernel/Capacity/PDEC/ColumnCRT 或自足 SC-9。"
            "但全局同集对偶证书仍未闭合。最新最窄剩余是：持久 `Gamma` 的多桶同集 "
            "PDEC 对偶比较 `U_CRT^multi<L_PDEC^multi`，以及不持久 `Gamma` 的 "
            "`DenseOldHoleKernelCapacityPDECOrColumnCRT` 或 `SelfContainedKuznetsovLSAtomSC9`。"
        ),
    }
...
def write_markdown(result: dict[str, Any], path: Path) -> None:
    ...
    lines.extend(
        [
            ...
            "`U_CRT^multi<L_PDEC^multi`；以及无持久 `Gamma` 分支中的稠密旧洞选择核容量/PDEC/ColumnCRT "
