...
TABLE012_X87 = DOCS / "prime-matrix-strict-table012-x87-y-ln2-applicability-router.json"
CLAIM_STATUS = DOCS / "claim-status-table.md"
SOURCE_FILES = [HIGH_TAIL, PSI_THETA, THETA_TABLE, TABLE012_X87, CLAIM_STATUS]
...
def build_result() -> dict[str, Any]:
    ...
    table012_x87 = load_json(TABLE012_X87)
    ...
    theta_self_closed = (
        theta_table.get("theta_less_than_identity_to_8e11_self_contained_closed") is True
        or table012_x87.get("theta_less_than_identity_to_8e11_self_contained_closed") is True
    )
    theta_self_source = (
        "table_012 x87 independent archive"
        if table012_x87.get("theta_less_than_identity_to_8e11_self_contained_closed") is True
        else "legacy theta table router"
    )
    arithmetic_closed = (
    ...
    theta_low_closed = theta_external_closed or theta_self_closed
    ...
    self_contained_full_closed = (
        ...
        and high_tail_closed
        and psi_theta_closed
        and theta_self_closed
        and arithmetic_closed
    )
    full_closed = external_full_closed or self_contained_full_closed
    ...
    rows = [
        ...
        row(
            ...
            theta_low_closed,
            True,
            "对 0<x<=8e11，theta(x)<x，故 theta(x)-x<0<x/36260；低段现有外部表路线和 table_012-x87 自足路线。",
            theta_self_source if theta_self_closed else "external table lane",
        ),
        row(
            "LowSegmentThetaLtIdentityTo8e11SelfContained",
            theta_self_closed,
            theta_self_closed,
            "table_012 独立极值归档、区间预算和 x87 log-oracle 证书给出低段 theta<x 的作者侧自足输入。",
            theta_self_source if theta_self_closed else "ThetaLessThanIdentityFiniteTableTo8e11SelfContainedLedger",
        ),
        row(
        ...
        row(
            ...
            full_closed,
            True,
            "高尾、中段和低段三段输入齐备后，Dusart P5.1 全段 theta(x)-x<x/36260 闭合。",
            "theorem closed on external lane and self-contained table_012 lane",
        ),
        row(
            STRICT_SELF,
            self_contained_full_closed,
            self_contained_full_closed,
            "table_012 原始有限计算/hash 已由独立极值归档与 x87 log-oracle 证书内化，P5.1 全段 theta 上界在作者侧解析输入链中闭合。",
            "closed" if self_contained_full_closed else "internalize table_012 finite computation/hash",
        ),
        row(
    ...
    return {
        ...
        "status": (
            "dusart_p51_theta_upper_full_external_and_self_contained_table_lane_closed"
            if self_contained_full_closed
            else "dusart_p51_theta_upper_full_external_lane_closed_self_contained_finite_table_remains_open"
        ),
        "same_theorem_target_preserved": True,
        "no_theorem_switch": True,
        "counterexample_assumption_only": True,
        "empirical_absence_not_used": True,
        "dusart_p51_full_theta_statement_closed": full_closed,
        ...
        "theta_minus_identity_upper_1_over_36260_for_all_x_self_contained_closed": self_contained_full_closed,
        ...
        "theta_less_than_identity_to_8e11_self_contained_source": theta_self_source,
        ...
        "theorem_statement_self_contained_table_lane": (
            "For every x>0, theta(x)-x < x/36260, using FK b0=28 high tail, "
            "middle psi node archive plus psi-theta finite audit, and the independent table_012 x87 archive "
            "for theta(x)<x up to 8e11."
        ),
        "next_direct_attack_target": DSTRUCTURE,
        "parallel_attack_targets": [] if self_contained_full_closed else [STRICT_SELF],
        "plain_conclusion": (
            "Dusart P5.1 的全段 theta 上界已同步到 table_012-x87 自足低段：低段由独立 "
            "`theta(x)<x` 到 `8e11` 归档给出，中段由 `psi<1.00002841x` 与 "
            "`psi-theta>0.9999sqrt(x)` 拼接给出，高尾由 FK b0=28 的 "
            "`0.00001262<1/36260` 给出。因此可在解析输入链中登记 "
            "`theta(x)-x<x/36260` 对所有 `x>0` 成立。该步仍不产生行/列反例链终端矛盾。"
        ),
        "rows": rows,
