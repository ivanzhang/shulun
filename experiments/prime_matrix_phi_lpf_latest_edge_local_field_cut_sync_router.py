#!/usr/bin/env python3
"""生成 Phi-LPF 最新 edge-local field-cut 同步证书。

用法示例：
  python3 experiments/prime_matrix_phi_lpf_latest_edge_local_field_cut_sync_router.py
  python3 -m json.tool docs/monograph/prime-matrix-phi-lpf-latest-edge-local-field-cut-sync-router.json
...
SLUG = "prime-matrix-phi-lpf-latest-edge-local-field-cut-sync"
OUT_LEDGER = DATA / f"{SLUG}-ledger.json"
OUT_JSON = DOCS / f"{SLUG}-router.json"
OUT_MD = DOCS / f"{SLUG}-router.md"

LATEST_NO_SWAP_CERT = DOCS / "prime-matrix-phi-lpf-latest-two-prime-no-swap-sync-router.json"
...
def source_atoms() -> str:
    """返回 latest 基底中替换 common packet 的 source 三原子。"""
    ...
    return [
        {
            ...
            "meaning": "edge-local formula 先剥离 LPF/Phi/Ferrers 无符号 edge label，剩余才是 signed atom fields 或命名 return。",
        },
        {
        ...
        {
            "from": "latest carried source packet",
            "to": source_atoms(),
            "meaning": "最新 no-swap 基底已经把 common packet 义务显式携带为 source 三原子。",
        },
        {
            "from": EDGE_LOCAL_FORMULA,
            "to": (
                f"{SIGNED_FIELD_TABLE} plus {OFFDIAG_ORIENTATION}, {OFFDIAG_EXACTUV}, "
                f"{INTERNAL_TRANSITION}, and carried source atoms"
            ),
            "meaning": "最新 edge-local 入口被收窄为逐 edge signed fields/return 与配套字段。",
        },
    ]
...
def build_rows(
    ...
    return [
        row(
            "LatestEdgeLocalFormulaImported",
            latest.get("next_primary_attack_target") == EDGE_LOCAL_FORMULA,
            False,
            "上一层最新 no-swap sync 已把直接硬点定位到 edge-local formula-or-return。",
            EDGE_LOCAL_FORMULA,
        ),
        row(
            ...
            True,
            "既有 edge-local field-cut 证书可直接作用在最新 edge-local 入口。",
            EDGE_LABEL_LEDGER,
        ),
        row(
        ...
        row(
            ...
            latest.get("paired_required_attack_targets", [])[-1] == atoms
            and (
        ...
        row(
            "LatestBasisReplacesEdgeLocalFormulaWithSignedAtomFields",
            field_cut.get("next_primary_attack_target") == SIGNED_FIELD_TABLE,
            False,
            "最新 edge-local formula 被收窄为 signed atom fields 或命名 return tag。",
            SIGNED_FIELD_TABLE,
        ),
        row(
        ...
        row(
            ...
            latest.get("paired_required_attack_targets", [None, None, None])[2]
            == INTERNAL_TRANSITION,
            False,
        ...
        row(
            ...
            f"{SIGNED_FIELD_TABLE} AND {OFFDIAG_ORIENTATION} AND {OFFDIAG_EXACTUV} AND {INTERNAL_TRANSITION} AND {atoms}",
        ),
    ]


def build_certificate() -> dict[str, Any]:
    """组装最新 edge-local field-cut 同步证书。"""
    latest = load_json(LATEST_NO_SWAP_CERT)
    ...
    return {
        "certificate_type": "prime_matrix_phi_lpf_latest_edge_local_field_cut_sync_router",
        "status": "phi_lpf_latest_edge_local_formula_synced_to_unsigned_field_cut_open",
        "same_theorem_target_preserved": True,
        "no_theorem_switch": True,
        "frontier_sync_only": True,
        "finite_evidence_not_used_as_global_proof": True,
        "latest_edge_local_formula_imported": rows[0]["closed"],
        "field_cut_router_imported": rows[1]["closed"],
        "closed_unsigned_edge_label_ledger_synced": rows[2]["closed"],
        "edge_atom_multiplicity_one_synced": rows[3]["closed"],
        "lpf_phi_unsigned_scope_exhausted_for_edge_local": rows[4]["closed"],
        "source_atoms_carried_forward": rows[5]["closed"],
        "latest_basis_replaces_edge_local_formula_with_signed_atom_fields": rows[6]["closed"],
        ...
        "plain_conclusion": (
            "本步把最新 `PhiLPFEdgeLocalTwoPrimeSignedInteractionFormulaOrReturnBeforePushforward` "
            ...
            "并仍需 orientation、ExactUV return、internal transition 与 source 三原子。"
        ),
    }
...
def render_markdown(cert: dict[str, Any]) -> str:
    """渲染 Markdown 证书。"""
    lines = [
        "# Prime Matrix Phi-LPF latest edge-local field-cut sync 证书",
        "",
        f"**状态：** `{cert['status']}`",
        "",
        ...
        "",
        "```text",
        f"latest_edge_local_formula_imported={fmt_bool(cert['latest_edge_local_formula_imported'])}",
        ...
        f"latest_basis_replaces_edge_local_formula_with_signed_atom_fields={fmt_bool(cert['latest_basis_replaces_edge_local_formula_with_signed_atom_fields'])}",
