...
def render_markdown(result: dict[str, Any]) -> str:
    ...
    lines.append("```")
    lines.append("")
    lines.append("## 7. 迭代审计更新")
    lines.append("")
    lines.append("后续审计把 `2n+1` 推广为")
    lines.append("")
    lines.append("```text")
    lines.append("m_t = 2^t n + (2^t-1),   t=1,2,3,4.")
    lines.append("```")
    lines.append("")
    lines.append("新增证书：")
    lines.append("")
    lines.append("```text")
    lines.append("experiments/prime_matrix_affine_power_two_phi_lpf_iteration_no_gain_audit.py")
    lines.append("data/prime-matrix-affine-power-two-phi-lpf-iteration-no-gain-ledger.json")
    lines.append("docs/monograph/prime-matrix-affine-power-two-phi-lpf-iteration-no-gain-audit.md")
    lines.append("docs/monograph/prime-matrix-affine-power-two-phi-lpf-iteration-no-gain-audit.json")
    lines.append("```")
    lines.append("")
    lines.append("结论：")
    lines.append("")
    lines.append("```text")
    lines.append("all_shifted_residue_formula_verified=true")
    lines.append("iteration_creates_new_phi_lpf_information=false")
    lines.append("euler_product_half_main_error_proved=false")
    lines.append("phi_lpf_parity_barrier_globally_broken=false")
    lines.append("```")
    lines.append("")
    lines.append("迭代只把 `p | m_t` 变成另一个 shifted residue 类：")
    lines.append("")
    lines.append("```text")
    ...
    lines.append("")
    lines.append(
        "naive gap 从约 `1/2` 变成约 `1-2^{-t}`，说明半主项不是 truncation error，"
        "而是漏掉固定 `2^t`-adic 样本空间。归一化后仍需从 affine rough survivors 中提取素数。"
    )
    lines.append("")
    return "\n".join(lines)
