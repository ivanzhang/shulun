#!/usr/bin/env python3
"""AlphaTail 小素平方命中重数乘积上界审计。

用法示例：
  python3 experiments/prime_matrix_alpha_tail_square_hit_bound_audit.py --selected '997:4096:-36,5003:8192:-36' --format table
"""
...
def audit_item(prime_bound: int, block: int, shift: int, alpha: float) -> dict:
    ...
    for point_count in (4, 5):
        ...
        for value in range(domain_start, domain_stop + 1):
            ...
            first_moment += hits
            actual_hmax = max(actual_hmax, hits)
        factor = main_factor(cutoff, z_value, shift, point_count)
        rough_surplus = rough_count - domain_size * factor
        square_capacity = square_hit_capacity(z_value)
        h_bound = point_count * square_capacity
        lower_bound = first_moment / h_bound if h_bound else 0.0
        rows.append(
            {
                "m": point_count,
                "z": z_value,
                "L2": square_capacity,
                "Hbound": h_bound,
                "actual_hmax": actual_hmax,
                "S1": first_moment,
                "S1_over_Hbound": lower_bound,
                "rough_surplus": rough_surplus,
                "closes": lower_bound >= rough_surplus,
            }
        )
    return {
        "p": prime_bound,
...
def main() -> None:
    ...
    if args.format == "table":
        print("p block shift m z L2 Hbound actual_hmax S1 S1_Hbound rough_surplus closes", flush=True)
        for audit in audits:
            for row in audit["rows"]:
                print(
                    f"{audit['p']} {audit['block']} {audit['shift']} "
                    f"{row['m']} {row['z']} {row['L2']} {row['Hbound']} "
                    f"{row['actual_hmax']} {row['S1']} {row['S1_over_Hbound']:.6f} "
                    f"{row['rough_surplus']:.6f} {row['closes']}",
                    flush=True,
                )
        return
    for audit in audits:
