#!/usr/bin/env python3
"""生成 Phi-LPF 最新 offdiagonal source-tuple 到 pure-pair atom 的同步证书。

用法示例：
  python3 experiments/prime_matrix_phi_lpf_latest_offdiagonal_pure_pair_atom_sync_router.py
  python3 -m json.tool docs/monograph/prime-matrix-phi-lpf-latest-offdiagonal-pure-pair-atom-sync-router.json
...
SLUG = "prime-matrix-phi-lpf-latest-offdiagonal-pure-pair-atom-sync"
OUT_LEDGER = DATA / f"{SLUG}-ledger.json"
OUT_JSON = DOCS / f"{SLUG}-router.json"
OUT_MD = DOCS / f"{SLUG}-router.md"

LATEST_TUPLE_CERT = DOCS / "prime-matrix-phi-lpf-latest-offdiagonal-seed-tuple-sync-router.json"
...
def source_atoms() -> str:
    """返回 common packet 在最新基底中携带的 source 三原子。"""
    ...
    return [
        {
            ...
            "meaning": "source tuple signed formula 的 first-seed 部分强制落到 tail=1 pure pair；tail>1 是 continuation lift。",
        },
        {
            "from": TAIL_LIFT,
        ...
        },
        {
            "from": COMMON_PACKET,
            "to": source_atoms(),
            "meaning": "pure-pair 路线中的 common packet 义务在最新基底中仍由 source 三原子携带。",
        },
        {
            "from": OFFDIAG_SIGNED_FORMULA,
            "to": (
            ...
            "meaning": "最新 source tuple formula 被收窄为 pure-pair signed atom 与配套 signed/ExactUV/transition/source 字段。",
        },
    ]
...
def build_rows(
    ...
    """生成最新 pure-pair atom 同步判定表。"""
    atoms = source_atoms()
    return [
        row(
            "LatestSourceTupleSignedFormulaImported",
            latest.get("next_primary_attack_target") == OFFDIAG_SIGNED_FORMULA,
            False,
            "上一层最新 tuple sync 已把直接硬点定位到 source tuple signed seed formula。",
            OFFDIAG_SIGNED_FORMULA,
        ),
        row(
            ...
            True,
            "既有 pure semiprime atom 证书可直接作用在最新 source tuple formula 入口。",
        ...
        row(
            ...
            False,
            "最新 source tuple formula 被收窄为 pure-pair signed atom 与 tail-lift/internal-transition 配套。",
            PURE_SEMIPRIME_ATOM,
        ),
        row(
        ...
        row(
            ...
            False,
            False,
            "本步只同步 pure-pair atom 拆解；未证明三命题无条件闭合。",
            f"{PURE_SEMIPRIME_ATOM} AND {OFFDIAG_ORIENTATION} AND {OFFDIAG_EXACTUV} AND {INTERNAL_TRANSITION} AND {atoms}",
        ),
    ]


def build_certificate() -> dict[str, Any]:
    """组装最新 pure-pair atom 同步证书。"""
    latest = load_json(LATEST_TUPLE_CERT)
    ...
    return {
        "certificate_type": "prime_matrix_phi_lpf_latest_offdiagonal_pure_pair_atom_sync_router",
        "status": "phi_lpf_latest_offdiagonal_source_tuple_synced_to_pure_pair_atom_open",
        "same_theorem_target_preserved": True,
        "no_theorem_switch": True,
        "frontier_sync_only": True,
        "finite_evidence_not_used_as_global_proof": True,
        "latest_source_tuple_signed_formula_imported": rows[0]["closed"],
        "pure_pair_atom_router_imported": rows[1]["closed"],
        "pure_pair_atom_bijection_synced": rows[2]["closed"],
        "tail_lift_phi_minus_one_synced": rows[3]["closed"],
        "tail_lift_no_new_first_seed_closed": rows[4]["closed"],
        "source_atoms_carried_forward": rows[5]["closed"],
        "latest_basis_replaces_tuple_formula_with_pure_atom": rows[6]["closed"],
        ...
        "plain_conclusion": (
            "本步把最新 `PhiLPFOffDiagonalOrderedSemiprimeSourceTupleSignedSeedFormulaBeforePushforward` "
            ...
            "signed seed atom，并仍需 orientation、ExactUV return、internal transition 与 source 三原子。"
        ),
    }
...
def render_markdown(cert: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "",
        "```text",
        f"latest_source_tuple_signed_formula_imported={fmt_bool(cert['latest_source_tuple_signed_formula_imported'])}",
