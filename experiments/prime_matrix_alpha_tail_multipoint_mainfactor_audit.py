#!/usr/bin/env python3
"""AlphaTail 多点共振主筛因子常数审计。

用法示例：
  python3 experiments/prime_matrix_alpha_tail_multipoint_mainfactor_audit.py --selected '997:4096:-36,5003:8192:-36' --format table
"""
...
def main_factor(prime_bound: int, block: int, shift: int, alpha: float, point_count: int) -> tuple[float, int]:
    """计算大素删除主因子 V_m 及 z。"""
    cutoff = int(alpha * prime_bound)
...
def audit_item(prime_bound: int, block: int, shift: int, alpha: float) -> dict:
    ...
    for point_count in (4, 5):
        ...
        factor, z_value = main_factor(prime_bound, block, shift, alpha, point_count)
        denominator = domain_size * factor
        required_constant = count / denominator if denominator else 0.0
        rows.append(
            {
                "m": point_count,
                "domain_size": domain_size,
                "count": count,
                "y": int(alpha * prime_bound),
                "z": z_value,
                "main_factor": factor,
...
def main() -> None:
    ...
    if args.format == "table":
        print("p block shift m domain count y z V C_req", flush=True)
        for audit in audits:
            for row in audit["rows"]:
                print(
                    f"{audit['p']} {audit['block']} {audit['shift']} "
                    f"{row['m']} {row['domain_size']} {row['count']} "
                    f"{row['y']} {row['z']} {row['main_factor']:.8f} "
                    f"{row['required_constant']:.6f}",
                    flush=True,
                )
        return
    for audit in audits:
