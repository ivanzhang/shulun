#!/usr/bin/env python3
"""生成 Phi-LPF latest constructor moving-atom 到逐点 signed 表的同步证书。

用法示例：
  python3 experiments/prime_matrix_phi_lpf_latest_new_joint_constructor_moving_atom_signed_table_sync_router.py
  python3 -m json.tool docs/monograph/prime-matrix-phi-lpf-latest-new-joint-constructor-moving-atom-signed-table-sync-router.json
...
SLUG = "prime-matrix-phi-lpf-latest-new-joint-constructor-moving-atom-signed-table-sync"
OUT_LEDGER = DATA / f"{SLUG}-ledger.json"
OUT_JSON = DOCS / f"{SLUG}-router.json"
OUT_MD = DOCS / f"{SLUG}-router.md"

CURRENT_MOVING = DOCS / (
    "prime-matrix-phi-lpf-latest-new-joint-constructor-alpha-terminal-three-atoms-sync-router.json"
)
    ...
    return [
        {
            ...
            "to": MOVING_ATOM,
            "meaning": "constructor latest 版本仍以排斥 actual clean-core moving atom 为目标。",
        },
        {
            "from": MOVING_ATOM,
            "to": LPF_PREIMAGE,
...
def build_rows(
    ...
    return [
        row(
            "CurrentConstructorMovingAtomFrontierImported",
            ...
            False,
            "刚提交的 constructor alpha terminal 三原子层把最窄直接主攻钉在 independent moving atom。",
            INDEPENDENT_MOVING,
        ),
        row(
        ...
        row(
            ...
            False,
            False,
            "本层只把 constructor moving atom 归约到逐点 signed 表、signed survival/row-mass 或速率型终端包；没有排斥 moving atom。",
...
def build_certificate() -> dict[str, Any]:
    ...
    cert = {
        "certificate_type": "prime_matrix_phi_lpf_latest_new_joint_constructor_moving_atom_signed_table_sync_router",
        "status": "phi_lpf_latest_constructor_moving_atom_synced_to_pointwise_signed_table_open",
        ...
        "current_constructor_moving_atom_frontier_imported": rows[0]["closed"],
        ...
        "plain_conclusion": (
            f"本步把当前 constructor latest 的 `{INDEPENDENT_MOVING}` 接到 moving-atom 下游两层："
...
def render_markdown(cert: dict[str, Any]) -> str:
    """渲染 Markdown 证书。"""
    lines = [
        "# Prime Matrix Phi-LPF latest constructor moving-atom signed-table sync 证书",
        "",
        f"**状态：** `{cert['status']}`",
        "",
        ...
        "```text",
        (
            "current_constructor_moving_atom_frontier_imported="
            f"{fmt_bool(cert['current_constructor_moving_atom_frontier_imported'])}"
        ),
        ...
        (
            "latest_moving_atom_exclusion_after_signed_split_proved="
            f"{fmt_bool(cert['latest_moving_atom_exclusion_after_signed_split_proved'])}"
        ),
