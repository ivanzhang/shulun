...
**Tech Stack:** Python 3 标准库（`argparse`、`fractions`、`json`、`math`、`pathlib`、`unittest`）；模块内实现最小精确 `mobius_value` 与 `euler_phi`，避免依赖当前工作树的未提交文件。

> **执行状态（2026-08-17）：** Task 1--6 均已实施并复核；有限残差使用精确 `Fraction` 零值，合同防火墙额外拒绝未许可来源和未核验的额外 `claimed_uses` 义务。
...
def mobius_value(index: object) -> int:
    """以有限试除法计算 Möbius 函数。"""
    # 拒绝非内建正整数；检测平方因子后返回 -1、0 或 1。

def euler_phi(index: object) -> int:
    """以有限试除法计算 Euler--φ。"""
    # 拒绝非内建正整数；逐素因子执行 phi 的标准有限公式。
