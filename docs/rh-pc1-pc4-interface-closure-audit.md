...
- 过密分支：PC4-Dual 经 `Dual-Gap-Ledger` 与 `DGap-Decomposition` 转成 ACC 负向同步、overlap 过剩、对偶短簇、对偶 PI、对偶 FCT、LV/LSMP 或接口失败；
...
4. 把 `DGap-Decomposition` 的盒分解有限重叠（见 `docs/rh-pc4-dual-box-overlap.md`）、投影正交化与低维频率抽取完全无条件化；
