...
| --- | --- | --- | --- | --- |
| `LatestRebasedSourceEntropyImported` | `true` | `false` | 上一层 rebase 已把第一硬点压到 actual source-domain entropy。 | ActualPreCauchySourceDomainAbsoluteEntropyLedger |
| `ExistingConstructorPayloadLoopCutReusable` | `true` | `false` | 旧 constructor source-entropy payload-loop cut 输入相同，可在新 rebase 前沿复用。 | FreshIndependentPreCauchyJointDeclarationLineOutsideConstructorPayloadLoop |
| `SourceEntropyAtomBoundaryCarried` | `true` | `true` | source entropy 原子化只给 signed row-mass entropy 包，不证明 signed law、row-mass 或 row support。 | NonzeroSignedRowSurvivalOnPhiLPFSupportBeforePushforward AND SameFormalUnitPrimitiveRowMassNormalizationAndNoHeavyRowLedger |
...
| file | sha256 |
| --- | --- |
| `experiments/prime_matrix_phi_lpf_latest_constructor_source_entropy_payload_loop_cut_rebase_sync_router.py` | `cc125449a0b33e0d878044d34031d73b0d0853bf4768b95606219a82d60d0f4a` |
