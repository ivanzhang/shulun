...
explicit_positive_terminal_budget_margin_imported=true
...
| --- | --- | --- | --- | --- |
| `PositiveMarginFrontierImported` | `true` | `false` | 最新前沿已把保标签商熵亏损同步为同参数正余量 D_prefix-E_named-U_cold>0。 | ExplicitPositiveTerminalBudgetMarginInequality |
| `CoupledMarginSchemaClosed` | `true` | `true` | D0、E0、U0、margin_delta 与 finite hash 的字段合同已列全。 | ConcreteSameParameterMarginTableCertificate |
| `ConcreteCandidateRowGenerated` | `true` | `false` | alpha=0.43, P>=100000 的同参数候选行已生成，但 D0/E0/U0 仍不是数值证书。 | fill D0/E0/U0 under same parameter_id |
| `RosserSawtoothFailureCharged` | `true` | `false` | Rosser floor/sawtooth 失败不再是无名 D0 损失，已强制登记为同 formal unit 命名终端收费。 | FiniteBoundaryPrefixRoughCountCertificate AND (NamedReturnSameParameterDeductionTable OR PDEC_CAP_OR_INTERNAL_CleanKLS_LargeSieve_FOR_AcyclicNoncanonicalTerminalFamily) |
| `D0FinitePrefixHashStillAbsent` | `true` | `false` | 渐近或外部尾段不能替代有限边界 prefix 证书；没有 hash 就不能给全局 D0。 | FiniteBoundaryPrefixRoughCountCertificate |
| `NamedReturnSameParameterSchemaClosed` | `true` | `false` | E_named 只剩持久终端家族排斥与非持久预算吸收两类来源；schema 闭合但数值扣除未证。 | PDEC_CAP_OR_INTERNAL_CleanKLS_LargeSieve_FOR_AcyclicNoncanonicalTerminalFamily AND SparseHistoryDemandExceedsNonpersistentSupplyBudget |
| `PersistentReturnReducedToTerminalFamily` | `true` | `false` | 持久命名回流只有 strict acyclic 终端家族被排斥后才能在 E0 中记为 0。 | PDEC_CAP_OR_INTERNAL_CleanKLS_LargeSieve_FOR_AcyclicNoncanonicalTerminalFamily |
