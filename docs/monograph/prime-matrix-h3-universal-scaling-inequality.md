...
该文件从理论上解释 `q/log q` 量级的来源，而不是再依赖数据拟合。令

\[
\Pi_1(q)=\pi(q)
\]

为方阵第一行内的素数个数，则由素数定理 `Pi_1(q)~q/log q`。另一方面，由相邻壳层单点性，
`q^2` 内旧 `p`-筛幸存者除端点 `q^2` 外就是素数；因此 H3 行平均满足
...
这里的 `1/2` 来自 `log(q^2)=2log q`。所以 H3 的 `q/log q` 尺度是第一行素数尺度在平方根壳层的
半密度投影。该桥接解释全局平均和自然尺度；逐行正下界仍需统一缺陷排斥。
