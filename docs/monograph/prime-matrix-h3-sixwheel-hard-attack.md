...
该文件直接攻尾标签/双粗补洞硬核。若连续 H3 候选

\[
a_j=\ell_jm_j,\qquad y<\ell_j=P^-(a_j)\le p,\quad P^-(m_j)\ge\ell_j
\]

由尾标签解释，则相邻候选差值 `2` 或 `4` 强制两侧全部大因子族互素；二步差值 `6` 也强制互素。
同一尾标签复用的候选索引间距至少 `y/4`。因此任意直径小于 `y` 的尾补洞块若含 `K` 个候选，
至少消耗 `2K` 个互异大素因子。三连补洞的最小单元是互素 `y`-rough 合数短差值方程
`2,4,6` 的重叠系统。

该局部刚性还未给出全局矛盾；剩余是证明这些短差值单元不能沿整行连续拼接而不触发
ColumnCRT、端点相位或互补商容量矛盾。
