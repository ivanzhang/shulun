...
| PDEC-CAP同集全局对偶前沿 | 新增/压到两侧终端估计 | 新增 `experiments/prime_matrix_pdec_cap_same_set_global_dual_router.py` 与 `docs/monograph/prime-matrix-pdec-cap-same-set-global-dual-router.md/json`。审查确认 `closed_current_materialized_pdec_gates=true`：当前 DualCap 质量来源与早期出口、PersistentCap 晋升删除势、ForcedCap 多桶 APS 路由、LHB 升层投影单调、PDEC cap 无循环与有限塔证据均已接线。新增 `experiments/prime_matrix_profinite_actual_payment_stitching_router.py` 与 `docs/monograph/prime-matrix-profinite-actual-payment-stitching-router.md/json` 后，`ProfiniteActualPaymentStitchingDichotomy` 也已闭合为投影塔二分逻辑。全局 PDEC-CAP 仍未闭合；当前最窄硬点更新为 `SameSetPDECDualComparisonForPersistentMFU_OR_OccupancySaturationOrSC9`：持久 `Gamma` 需证明多桶同集 PDEC 对偶比较 `U_CRT^multi<L_PDEC^multi`；不持久 `Gamma` 已由 diffuse 终端分裂、支撑耗尽桥与 HRO 路由继续压到 `OccupancySaturationPDECOrColumnCRT` 或自足 `SC-9`。 |
| PDEC-CAP diffuse终端分裂 | 新增/不持久分支继续压窄 | 新增 `experiments/prime_matrix_pdec_cap_diffuse_terminal_split_router.py` 与 `docs/monograph/prime-matrix-pdec-cap-diffuse-terminal-split-router.md/json`。审查确认 `diffuse_terminal_split_closed=true`、`external_deep_theorem_version_closed_if_accepted=true`、`self_contained_diffuse_terminal_closed=false`。该项把不持久 `Gamma` 分支的宽口径 `FiberDeletion/NoDeletion-KL/CleanKLS` 合成为严格二分：持续删除经支撑耗尽桥和 HRO 下界路由只剩 `OccupancySaturationPDECOrColumnCRT`；删除停止且 KL/MI 偏斜回流 refined/new-layer PDEC；KL/MI 平坦才进入 CleanKLS，且 K1--K9 失败回流命名出口。完全自足剩余为 `SelfContainedKuznetsovLSAtomSC9`，不能把外部 KLS 输入版误称为自足闭合。 |
