...
| C13小余量有限化归约 | 新增/Allow<224进入有限表 | 新增 `docs/monograph/prime-matrix-eda-alpha-tail-tailpair-c13-small-slack-finite-reduction-contract.md` 与脚本 `experiments/prime_matrix_alpha_tail_tailpair_c13_small_slack_finite_reduction_contract.py`。把源槽付款分为 `Allow>=224` 大余量自动付款与 `Allow<224` 有限证书出口。当前样本 `large_slack_windows=1, small_slack_windows=1, all_pass=True, F_small={(5003,8192,-36)}`。全局剩余精确为证明完整目标族中所有 `Allow<224` 窗口均落入有限表。 |
