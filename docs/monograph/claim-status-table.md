...
| PDEC-CAP同集全局对偶前沿 | 新增/压到primitive多原子证书 | 新增 `experiments/prime_matrix_pdec_cap_same_set_global_dual_router.py` 与 `docs/monograph/prime-matrix-pdec-cap-same-set-global-dual-router.md/json`。审查确认 `closed_current_materialized_pdec_gates=true`：当前 DualCap 质量来源与早期出口、PersistentCap 晋升删除势、ForcedCap 多桶 APS 路由、LHB 升层投影单调、PDEC cap 无循环与有限塔证据均已接线。新增 `experiments/prime_matrix_profinite_actual_payment_stitching_router.py` 与 `docs/monograph/prime-matrix-profinite-actual-payment-stitching-router.md/json` 后，`ProfiniteActualPaymentStitchingDichotomy` 也已闭合为投影塔二分逻辑。新增持久有限签名统一、SC-9 边界调和与持久终端准入后，全局 PDEC-CAP 仍未闭合；当前最窄硬点更新为 `PrimitiveMultiAtomSameFormalUnitPDECCertificate`：只剩准入后的 primitive 多原子同 formal unit PDEC 容量证书。 |
