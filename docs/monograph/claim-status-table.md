...
| C13低筛AP-Brun常数余量 | 新增/条件常数接口 | 新增 `docs/monograph/prime-matrix-eda-alpha-tail-tailpair-c13-lowsieve-ap-brun-margin.md` 与脚本 `experiments/prime_matrix_alpha_tail_tailpair_c13_lowsieve_ap_brun_margin.py`。定义 AP 删除尺度 `B_AP=sum S(g)|J∩a mod ell|/log^2 J_-`；若目标删除类满足 `U_AP<=C_AP B_AP` 且 `C_AP B_AP<=G_geom-R`，则低筛保存闭合。当前高 P 样本 `B_AP=51.228906, slack=2537.963717, allowable_C=49.541634`；逐窗口最紧 `p=5003` 允许 `27.803171`，逐层最紧 `p=5003,m=5` 允许 `27.077992`，所以条件输入 `C_AP<=20` 在样本中通过。该项不证明 `C_AP<=20`，剩余是 AP-Brun 常数包或超标类 PDEC/SAE。 |
