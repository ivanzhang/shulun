...
| Prime Matrix EDA AlphaTail TailPair shortgap certificate | 短差值尾素对容量证书 | 已把等乘数共振允许 gap 集显式化为 `G_r(m)={|jr|/u:1<=j<m,u|jr}`，并给出固定窗口精确证书 `M2_equal<=m^2 sum_{g in G_r(m)}#{q,q+g in P_T}`。新增脚本精确计数短差值尾素对并核验样本 `upper_pass=True` | `docs/monograph/prime-matrix-eda-alpha-tail-tailpair-shortgap-certificate.md`, `experiments/prime_matrix_alpha_tail_tailpair_shortgap_certificate_audit.py` | 否，有限窗口证书已证；全局仍需 Brun/Selberg 固定差值素对常数包或 SAE/Endpoint 排斥 |
