...
| Prime Matrix EDA AlphaTail TailPair local constant sensitivity | 固定 gap 局部常数阈值 | 新增局部常数灵敏度账本：样本 `6` 行中 `max_required_C_BS=0.857166`、`max_local_required_C=1.292474`；`C_local=1.2` 时有 `41` 个 EndpointSpike，`C_local=1.254` 时仍有 `7` 个，`C_local=1.3` 时全部清空。该项说明样本端点链条可由 `C_local=1.3` 吸收，但全局仍需证明固定 gap 短区间常数包或提交 SAE | `docs/monograph/prime-matrix-eda-alpha-tail-tailpair-local-constant-sensitivity.md`, `experiments/prime_matrix_alpha_tail_tailpair_local_constant_sensitivity.py` | 否，样本阈值已抽取；仍需全局固定 gap 常数 `<=1.3`，或有限/可求和 SAE 证书 |
