...
| 行列无条件自足前沿路由 | 新增/最窄硬点重定位 | 新增 `experiments/prime_matrix_row_column_unconditional_frontier_router.py` 与 `docs/monograph/prime-matrix-row-column-unconditional-frontier-router.md/json`。路由裁定 `row_column_unconditional_closed=false`、`canonical_source_boundary_closed=true`、`generic_unrestricted_self_contained_refuted=true`、`terminal_triad_routed_no_fourth_exit=true`。当前最窄硬点仍为 `A1-FO-PDEC-SameFormalUnit`，但嵌套单位重复、fractional weighted full duplicate 恢复路线、以及当前 cross-q 坐标图持久化路线均已阻断；下一步转为 physical/primitive `U_CRT<1.9997507790353146` 或 SAE/Endpoint 吸收。 |
