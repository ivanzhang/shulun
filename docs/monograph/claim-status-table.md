...
| PDEC-CAP同集全局对偶前沿 | 新增/压到APS投影塔二分 | 新增 `experiments/prime_matrix_pdec_cap_same_set_global_dual_router.py` 与 `docs/monograph/prime-matrix-pdec-cap-same-set-global-dual-router.md/json`。审查确认 `closed_current_materialized_pdec_gates=true`：当前 DualCap 质量来源与早期出口、PersistentCap 晋升删除势、ForcedCap 多桶 APS 路由、LHB 升层投影单调、PDEC cap 无循环与有限塔证据均已接线。全局 PDEC-CAP 仍未闭合；当前最窄硬点更新为 `ProfiniteActualPaymentStitchingDichotomy`。持久 `Gamma` 进入多桶同集 PDEC 对偶比较 `U_CRT^multi<L_PDEC^multi`；不持久则必须由 FiberDeletion、NoDeletion-KL/PDEC 或 KL 平坦的 CleanKLS/DLS 吸收。 |
