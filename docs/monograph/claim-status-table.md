...
| 行列无条件自足前沿路由 | 新增/最窄硬点重定位 | 新增 `experiments/prime_matrix_row_column_unconditional_frontier_router.py` 与 `docs/monograph/prime-matrix-row-column-unconditional-frontier-router.md/json`。路由裁定 `row_column_unconditional_closed=false`、`canonical_source_boundary_closed=true`、`generic_unrestricted_self_contained_refuted=true`、`terminal_triad_routed_no_fourth_exit=true`。当前最窄硬点为 `A1-FO-PDEC-SameFormalUnit`：嵌套单位重复已被支配，剩余是 fractional Weighted Hall dual、coordinate quotient / primitive PDEC threshold、exact duplicate SAE/Endpoint absorption 或 cross-q persistence/rejection。 |
