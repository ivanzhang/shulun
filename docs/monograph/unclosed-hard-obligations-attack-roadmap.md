...
新增显式 Fourier 阈值账本后，下一步硬点应固定为单一投影：
...
1. 为 `g_199` 写出 `mass/nonnegative` 与实际支撑约束；
2. 检查镜像、列容量、尾非复用是否能合法作用于同一 `g_199`；
3. 若不能给出 `U_CRT,199<3.959...`，则证明该投影只属稀疏端点，转 `SAE/Endpoint`。

新增对偶聚簇审计后，最小硬攻点更新为：

```text
DualCluster-Exclusion(199,95,arc length 11)
```

具体任务：

1. 若短弧聚簇在多个相邻/镜像/递归坏窗中复现，构造 `PDEC-Dual-Cert`；
