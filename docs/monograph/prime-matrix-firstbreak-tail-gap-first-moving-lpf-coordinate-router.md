...
**状态：** `first_moving_lpf_coordinate_pressure_reduced_to_single_coordinate_drift_open`

首移动 LPF 坐标压力被拆成稳定前缀后的有效宽度、dyadic 坐标层、活动坐标 product-width 出口与单坐标压力定位。若活动坐标乘积超过有效宽度，则回到 ColumnCRT/PDEC 或有限原子；若乘积不超过有效宽度，则活动个数有显式对数界，超界压力必须落在单个 mu。剩余变成单个 mu 随 P 漂移的压力，而不是匿名首移动坐标池。
...
- `SingleMovingLPFCoordinateDriftOrPrefixColumnCRTPDEC` 仍未闭合。
- 行/列命题仍未无条件闭合。
...
| file | sha256 |
| --- | --- |
| `experiments/prime_matrix_firstbreak_tail_gap_first_moving_lpf_coordinate_router.py` | `9f78dd4dd537cecd056d6b37439aac6ec9b016da01b0a6537a3b09cef3191fa9` |
