...
**状态：** `phi_lpf_latest_constructor_offdiagonal_seed_synced_to_tuple_fields_open`

本步把 constructor 最新 `PhiLPFOffDiagonalOrderedSemiprimeFirstSeedSignedTableBeforePushforward` 接入既有 tuple-fields 证书。LPF/Phi 桶恒等式已关闭 offdiagonal source tuple 的 owner、first rough prime、q-rough tail 与 Phi fiber mass；因此 offdiagonal seed 口不再是未解析黑箱。剩余不能由 LPF/Phi 反推，必须正向提交 source tuple signed seed formula、orientation parity/branch side、ExactUV fixed pair/return tag，并保留 internal transition、source 三原子、signed survival 与 row-mass/no-heavy-row。
...
| `PreCauchyActualNoncanonicalEmitterSourceDeclarationPacket` | `AlphaRowAnchorPhaseEmissionFormulaLedger AND IndependentNoncanonicalPreCauchyArithmeticIdentityStatementLedger AND SameUnitExactUVRankMultiplicityCertificateForPrimitiveKernelRows` | diagonal/common packet 义务继续由 source 三原子携带；本层不把 common packet 当作自证终点。 |
| `PhiLPFOffDiagonalOrderedSemiprimeFirstSeedSignedTableBeforePushforward` | `PhiLPFOffDiagonalOrderedSemiprimeSourceTupleSignedSeedFormulaBeforePushforward AND PhiLPFOffDiagonalSemiprimeOrientationParityAndBranchSideLawBeforePushforward AND PhiLPFOffDiagonalSemiprimeExactUVFixedPairAndReturnTagLedgerBeforePushforward plus PhiLPFInternalPrimeAdjoinSignedTransitionLawBeforePushforward, SameFormalUnitPrimitiveRowMassNormalizationAndNoHeavyRowLedger, signed survival and carried source atoms` | 剥去 LPF/Phi 无符号层后，constructor 最新基只保留真正 signed/ExactUV/source 义务。 |

## 2. 判定表

| gate | closed | proved | meaning | remaining |
| --- | --- | --- | --- | --- |
| LatestConstructorOffDiagonalSeedHardpointImported | `true` | `false` | 上一层 constructor semiprime seed diagonal sync 已把新增 seed 口定位到 offdiagonal 表。 | PhiLPFOffDiagonalOrderedSemiprimeFirstSeedSignedTableBeforePushforward |
| ConstructorSideGatesCarried | `true` | `false` | 本层只剥离 OFFDIAG_SEED 的无符号 tuple 字段；signed survival 与 row-mass/no-heavy-row 仍是 constructor 侧门。 | NonzeroSignedRowSurvivalOnPhiLPFSupportBeforePushforward AND SameFormalUnitPrimitiveRowMassNormalizationAndNoHeavyRowLedger |
| OffDiagonalTupleFieldsRouterImported | `true` | `true` | 既有 tuple-fields 证书可直接作用在 constructor 最新 offdiagonal seed 入口。 | PhiLPFOffDiagonalSemiprimeSourceTupleFieldLedgerBeforePushforward |
...
| LPFPhiUnsignedScopeExhausted | `true` | `true` | LPF/Phi 桶恒等式已用尽：它只给支撑、tuple 字段和容量，不能产生 signed coefficient。 | PhiLPFOffDiagonalOrderedSemiprimeSourceTupleSignedSeedFormulaBeforePushforward AND PhiLPFOffDiagonalSemiprimeOrientationParityAndBranchSideLawBeforePushforward |
| SourceAtomsCarriedForward | `true` | `false` | diagonal/common packet 义务在 constructor 最新基中仍以 source 三原子携带，尚未由本步证明。 | AlphaRowAnchorPhaseEmissionFormulaLedger AND IndependentNoncanonicalPreCauchyArithmeticIdentityStatementLedger AND SameUnitExactUVRankMultiplicityCertificateForPrimitiveKernelRows |
| LatestBasisReplacesOffdiagSeedWithTuplePayload | `true` | `false` | constructor 最新 offdiagonal seed 表被收窄为 source tuple signed formula 与配套字段。 | PhiLPFOffDiagonalOrderedSemiprimeSourceTupleSignedSeedFormulaBeforePushforward AND PhiLPFOffDiagonalSemiprimeOrientationParityAndBranchSideLawBeforePushforward AND PhiLPFOffDiagonalSemiprimeExactUVFixedPairAndReturnTagLedgerBeforePushforward |
...
| file | sha256 |
| --- | --- |
| `experiments/prime_matrix_phi_lpf_latest_constructor_offdiagonal_seed_tuple_sync_router.py` | `51cb205c8ea135a25e6cfe2c54d90ee79f8cdc4b13f2b8fd5831bdfffa19260d` |
