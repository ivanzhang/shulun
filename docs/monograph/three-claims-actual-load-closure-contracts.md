...
### 1.189 Phi-LPF latest offdiagonal seed tuple sync 更新
...
actual-load 含义是：最新
`PhiLPFOffDiagonalOrderedSemiprimeFirstSeedSignedTableBeforePushforward`
已接入既有 tuple-fields 拆解。LPF/Phi 对这个入口能提供的全部信息是
...
这些字段关闭了支撑、类型、tail 纤维与 occurrence 容量，但不产生 signed seed value、
orientation parity、branch side、local factor 或 ExactUV return tag。样本 `N=10000`
中 offdiagonal types 为 `2600`，tuple occurrences 与 Phi sum 同为 `5468`，tail
`1/>1` 分解为 `2600/2868`。
...
### 1.190 Phi-LPF latest offdiagonal pure-pair atom sync 更新
...
actual-load 含义是：最新
`PhiLPFOffDiagonalOrderedSemiprimeSourceTupleSignedSeedFormulaBeforePushforward`
已经接入 pure semiprime atom 拆解。每个 ordered type `(p,q), p<q` 的 first seed
原子是唯一 tail=1 的 pure pair：

```text
(p,q,1), value=p*q.
```

所有 `tail>1` occurrence 都不是新的 first seed；它们只提供

```text
Phi(floor(N/(p*q)),q)-1
```

的 continuation lift 质量，并交给 internal prime-adjoin transition compatibility。样本
`N=10000` 中 pure pair atoms 为 `2600`，tail lift 为 `2868`，合计 `5468`
个 offdiagonal occurrences。
...
### 1.191 Phi-LPF latest pure-pair Ferrers support sync 更新
...
actual-load 含义是：最新
`PhiLPFOffDiagonalPureSemiprimePairSignedSeedAtomBeforePushforward`
已接入 Ferrers support 拆解。pure pair 的支撑边集为

```text
p<q<=N/p,  p,q prime,
```

且随着 owner prime `p` 增大，右邻域嵌套下降；left/right degree 与总边数全由 prime table
和 `floor(N/p)` 决定。样本 `N=10000` 中 left owner layers 为 `25`，right prime
vertices 为 `668`，pure-pair edges 为 `2600`。
...
### 1.192 Phi-LPF latest two-prime no-swap sync 更新
...
actual-load 含义是：最新
`PhiLPFOffDiagonalTwoPrimeInteractionSignedKernelBeforePushforward`
已接入 ordered no-swap 拆解。LPF owner source domain 只保留 canonical ordered edge
`(p,q)` with `p<q`；交换对称
...
只说明同一整数值，不提供 reverse source row、signed cancellation partner 或第二条
pre-Cauchy signed lane。样本 `N=10000` 中 ordered edges 与 unordered products 均为
`2600`，reverse edges 为 `0`，duplicates 为 `0`。
...
### 1.203 Phi-LPF moving-atom signed-injection split 更新
...
actual-load 含义是：上一层已经把 moving atom 的无符号源前像固定为 LPF-owned buckets，所以
`LPFMovingAtomSignedPreimageMassInjectionLedger` 内部不再含新的筛计数自由度。若逐点
prepushforward same-`(u,v)` signed sum identity 已给出，则从 `|sum w_e| >= T` 抽取某一正/负
sign-lane 至少 `T/2` 的绝对质量只是有限线性代数。

真正缺失的仍是每个 LPF bucket support key 上的 signed coefficient、local factor、
alpha/delta branch 与推前前恒等式，并且必须配合同 formal unit 的 signed survival 与 row-mass/no-heavy-row。
因此最新直接主攻同步为：
...
并行保留 `PhiLPFBucketSignedCoefficientLawBeforePushforward`、
`ActualNoncanonicalPrimitiveSummandSignedWeightExpressionBeforePushforward`、signed survival、
...
### 1.213 Phi-LPF latest new-joint trace-exit source-rank sync 更新
...
actual-load 含义是：上一层留下的
`NewPrimitiveAtomicSignedPayloadOrTraceFormulaArtifact` 不是新的独立终点。若它不是
signed-lane 环内改名，就必须携带 actual source-rank/no-collapse 三原子：
...
既有 post-antisplit/source-rank 收敛证书说明，这些 source/no-collapse 线在同一
formal-unit 的逐 primitive alpha/delta 核表上汇合。因此当前 new-joint trace-exit 的最新
直接主攻推进为：
...
### 1.225 Phi-LPF latest constructor origin-cyclecut sync 更新
...
actual-load 含义是：constructor 口径的 primitive summand 来源恒等式若要闭合，必须给逐
actual primitive summand 的 clean-core 原始生成表，并由无环 pre-Cauchy source seed 的
signed row emitter 正向产生。沿现有 signed coordinate/source 语料展开会回到已登记闭环，
所以原环不能作为证明。

因此 latest 直接主攻同步为三破环口：
...
### 1.234 Phi-LPF latest constructor pure-pair atom sync 更新
...
actual-load 含义是：`PhiLPFOffDiagonalOrderedSemiprimeSourceTupleSignedSeedFormulaBeforePushforward`
中的 first seed 原子不是整条 `q`-rough tail family，而是唯一的 `tail=1` pure semiprime
pair `p*q`。`tail>1` 的质量
`Phi(floor(N/(p*q)),q)-1` 只进入 internal transition lift，不产生新的 first seed。
...
### 1.235 Phi-LPF latest constructor pure-pair Ferrers support sync 更新
...
actual-load 含义是：pure-pair atom 的支撑已经不是自由变量。边集由
`p<q<=N/p` 唯一决定，left/right degree 与 edge 总数由素数表和 `floor(N/p)` 支付。
Ferrers support 只关闭支撑、度数和容量，不发射 signed value、orientation 或 local factor。
...
### 1.238 Phi-LPF latest constructor signed atom trace-sync 更新
...
actual-load 含义是：constructor signed atom fields 不能由无符号 edge label 后验推出。
signed value、local factor、orientation/branch side、alpha/delta payload、ExactUV fixed pair
与 source row 必须落在同一个 pre-Cauchy trace key 上；缺失、冲突、零因子、超预算或跨 key
读取都进入 named return/PDEC。样本 `N=10000` 中仍是 `2600` 个 trace packets 与
`15600` 个开放 signed slots。
...
actual-load 含义是：上一层的 source-rank/terminal 三输入核表不是最终新变量。
既有 strict 三腿同步显示 alpha、weight、rank 分攻会回到同一个 primitive source
table 或 PDEC/CleanKLS 终端门；非递归核表若没有新的 actual joint constructor
公式，又经 joint-alpha/same-row/row-level 表返回终端叶子。当前终端叶子已经压到：
...
noncanonical legal mode 再被 actual-source bridge 证书过滤为：
...
外部引理版仍只在 `AcceptedFullSKLSExtExternalContract AND
...
actual-load 含义是：rate-bearing Mertens 尾段闭合后，内部线不应继续把
`ForwardAcyclicPreCauchySourceRootPacketOrNamedReturn` 当作活动证明原子。
后续 strict 证书已经显示 source-root 自足支路回到 signed/source-rank/alpha 终端环；
direct PDEC scope 支路在当前内部语料中饱和但未证明；KZ no-cycle 支路在禁止
NCBLK/source-root 复用后压到 KZ-E direct no-projection，并进一步压到 A1 clean branch
...
### 1.215 Phi-LPF strict-k low-carrier lower-half source-cut 更新
...
actual-load 含义是：上一层的 low-carrier high-prime payment 不是当前行自反馈。
若 `n=mr` 落在 strict 行 `kP<n<(k+1)P`，其中 `2<=m<=k` 且 `r>P` 为素数，则
...
因此每个 payment prime source 都来自当前目标行左侧的下半行源，而不是当前行、近顶端行
或未来行。并且固定目标行内，一个源素数 `r>P` 至多对应一个 carrier `m`；否则相邻
carrier 的乘积差至少为 `r>P`，超过行宽 `P-1`。
...
```text
all row slots = image(lower-half prime source injection) union P-smooth slots.
```
...
本层排除了 payment 来源环，但尚未证明下半源注入像与 `P`-smooth 槽不能完美铺满；行/列
命题仍未无条件闭合。

### 1.216 Phi-LPF strict-k payment Beatty source-map 更新
...
actual-load 含义是：上一层的下半源注入不是任意选择关系，而是一张唯一近倍数源表。对
`1<k<P`，源素数 `r>P` 能支付目标行当且仅当
...
并用大样本行确认 `P=571,k=438` 与 `P=1009,k=1008` 的 `H_m` payment 与 Beatty
source count 分别同为 `382` 与 `670`。零行反设进一步规范为：
