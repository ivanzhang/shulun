...
```text
name: NonTautologicalPDECOrCleanKLS
...
PDEC 是三终端中最直接连接 A1 已闭合边界的端口；当前已审计 FO-PDEC 强信号链已被逐层降口径，最后两个物理原子由同固定偏移纤维的本地素数见证吸收；已物化 LocalSurvivor/SAE 包总账也无开放窗口；已知 packet extractor 入口全覆盖，且无未命名新 sparse 入口。LocalSurvivor 当前分支只剩未来显式新增入口的条件义务；当前主硬点转向非二点 primitive PDEC 或 CleanKLS/DLS。
