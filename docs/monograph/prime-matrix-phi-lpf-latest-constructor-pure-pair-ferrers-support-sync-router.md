...
**状态：** `phi_lpf_latest_constructor_pure_pair_atom_synced_to_ferrers_support_open`

本步把 constructor 最新 `PhiLPFOffDiagonalPureSemiprimePairSignedSeedAtomBeforePushforward` 接入 Ferrers support 拆解。pure pair 支撑由 `p<q<=N/p` 完全决定，left/right degree 和 edge 总数已由 LPF/Phi/素数表支付；因此 pure-pair 剩余不再是支撑或容量问题，而是每条 `(p,q)` 边上的 two-prime signed interaction kernel，并仍需 orientation、ExactUV return、internal transition、source 三原子、signed survival 与 row-mass/no-heavy-row。
...
| from | to | meaning |
| --- | --- | --- |
| `PhiLPFOffDiagonalPureSemiprimePairSignedSeedAtomBeforePushforward` | `PhiLPFOffDiagonalPurePairFerrersSupportAndDegreeLedgerBeforePushforward AND PhiLPFOffDiagonalTwoPrimeInteractionSignedKernelBeforePushforward` | constructor pure pair atom 先剥离 Ferrers 支撑/度数账本，剩余才是每条边的 signed kernel。 |
...
| `PhiLPFOffDiagonalPureSemiprimePairSignedSeedAtomBeforePushforward` | `PhiLPFOffDiagonalTwoPrimeInteractionSignedKernelBeforePushforward plus PhiLPFOffDiagonalSemiprimeOrientationParityAndBranchSideLawBeforePushforward, PhiLPFOffDiagonalSemiprimeExactUVFixedPairAndReturnTagLedgerBeforePushforward, PhiLPFInternalPrimeAdjoinSignedTransitionLawBeforePushforward, SameFormalUnitPrimitiveRowMassNormalizationAndNoHeavyRowLedger, signed survival, and carried source atoms` | constructor 最新 pure-pair atom 被收窄为 two-prime signed interaction kernel 与配套字段。 |

## 2. 判定表

| gate | closed | proved | meaning | remaining |
| --- | --- | --- | --- | --- |
| LatestConstructorPurePairAtomImported | `true` | `false` | 上一层 constructor pure-pair sync 已把直接硬点定位到 pure semiprime pair signed atom。 | PhiLPFOffDiagonalPureSemiprimePairSignedSeedAtomBeforePushforward |
| ConstructorSideGatesCarried | `true` | `false` | 本层只替换 pure pair atom；signed survival 与 row-mass/no-heavy-row 仍是 constructor 侧门。 | NonzeroSignedRowSurvivalOnPhiLPFSupportBeforePushforward AND SameFormalUnitPrimitiveRowMassNormalizationAndNoHeavyRowLedger |
| FerrersSupportRouterImported | `true` | `true` | 既有 Ferrers support 证书可直接作用在 constructor 最新 pure-pair atom 入口。 | PhiLPFOffDiagonalPurePairFerrersSupportAndDegreeLedgerBeforePushforward |
...
| LatestBasisReplacesPureAtomWithTwoPrimeKernel | `true` | `false` | constructor 最新 pure-pair atom 被收窄为 two-prime signed interaction kernel 与配套字段。 | PhiLPFOffDiagonalTwoPrimeInteractionSignedKernelBeforePushforward |
...
| file | sha256 |
| --- | --- |
| `experiments/prime_matrix_phi_lpf_latest_constructor_pure_pair_ferrers_support_sync_router.py` | `5015ff4a4983b72162fc54dc96d3852683ec1a5c4921f5072332117e9f60a07e` |
