...
**状态：** `pdec_cap_diffuse_terminal_split_reduced_to_global_deletion_or_sc9`

不持久 Gamma 分支已经不再是 `FiberDeletion/NoDeletion/CleanKLS` 的宽口径黑箱。现有账本合成后，非终端门全部接线：持续删除进入删除势账本，删除停止时 KL/MI 偏斜回流 PDEC，只有 KL/MI 平坦才进入 CleanKLS；而 CleanKLS 的 K1--K9 失败项也全部回流命名出口。剩余自足硬点压成两项：全局删除势发散/支撑耗尽耦合，或 flat clean 分支的 SC-9 谱大筛原子。

## 1. 分裂律

For the nonpersistent Gamma branch, the inverse finite-signature tower gives diffuse payment. Along any same-source lift tower, either the deletion potential sum diverges and the support density is exhausted, or deletion is summable and the branch is NoDeletion. In NoDeletion, persistent KL or phase-residue mutual information returns to refined/new-layer PDEC; only the flat KL/MI case is admitted to CleanKLS/DLS. K1--K9 then route all non-clean failures back to PDEC/SAE/Multiplicity/Promotion. Thus the self-contained diffuse terminal is reduced to global deletion exhaustion or the named Kuznetsov-LS atom SC-9; an external DI/BFI/Kuznetsov input would close the flat clean branch only as an external-theorem version.
...
same-source lift tower
  => sum deletion potential diverges
       => support exhaustion / deletion contradiction obligation;
  or NoDeletion;
...
- `narrowest_diffuse_hardpoint=GlobalDeletionDivergenceOrSupportExhaustion_OR_SelfContainedKuznetsovLSAtomSC9`。
- `open_final_gates=['GlobalDeletionDivergenceOrSupportExhaustion', 'SelfContainedKuznetsovLSAtomSC9']`。
...
| `GlobalDeletionDivergenceOrSupportExhaustion` | `false` | `true` | finite tower evidence only | 还需证明任意 diffuse 反例塔若持续 FiberDeletion，则删除势发散确实耗尽可承担全行覆盖责任的支撑。 |
...
本路由器关闭的是 diffuse 分支中的无名逃逸和 NoDeletion-KL 第三出口。它不证明完整行/列无条件定理，也不证明 PDEC-CAP 同集全局对偶证书。完全自足版仍需提交 `GlobalDeletionDivergenceOrSupportExhaustion` 或 `SelfContainedKuznetsovLSAtomSC9` 的最终证明；外部深定理版必须明确登记所引用的 KLS/DI/BFI/Kuznetsov 输入。
