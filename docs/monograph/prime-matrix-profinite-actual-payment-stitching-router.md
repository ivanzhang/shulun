...
- `profinite_aps_dichotomy_closed=false`。
...
| --- | --- | --- | --- | --- |
| `PDECCapRequestsAPS` | `false` | `false` | SameSetPDECDualComparisonForPersistentMFU_OR_DiffuseCleanKLS | 上一层 PDEC-CAP 前沿已经把最窄逻辑门定位为 APS 投影塔二分。 |
...
| `ProfiniteActualPaymentStitchingDichotomyClosed` | `false` | `false` | finite projection compactness / pigeonhole dichotomy | APS 的全局二分逻辑闭合；这只排除无名 APS 出口，不证明两侧终端估计。 |
