...
$$
|E(P)|<\frac{3P}{4}-\frac{P}{8\log P}-\frac{0.05625\,P}{\log^2 P}.
$$
...
(4) 解：
$$
P-1-|E(P)|\ge \frac{P}{4}+\frac{P}{8\log P}+\frac{0.05625P}{\log^2 P}-0.62753.
$$

(5) 故
$$
|E(P)|<\frac{3P}{4}-\frac{P}{8\log P}-\frac{0.05625P}{\log^2 P}.\quad\square
$$
...
| $P$ | K3''' 上界 | K3'''' 上界 | 额外节省 $\sim 0.05625P/\log^2 P$ |
|---|---|---|---|
| 180 | $134.66$ | $134.27$ | $0.39$ |
| 503 | $367.05$ | $365.34$ | $1.71$ |
| 1009 | $738.06$ | $735.10$ | $2.96$ |
| 4999 | $3675.78$ | $3666.50$ | $9.28$ |
| 10007 | $7363.45$ | $7349.61$ | $13.84$ |

K3'''' 的额外节省**随 $P$ 增长** $\sim P/\log^2 P$——这正是 Dusart 三项展开的第三项贡献。
...
K3'''        |E|<3P/4 - P/(8 log P)                          + Dusart 2010 (1/(log P-1))
   ↓ 加 Dusart 2010 三项
K3''''       |E|<3P/4 - P/(8 log P) - 0.05625P/log^2 P       + Dusart 三项
   ↓ ?
...
- 全域空行密度 $|E(P)|/P< 3/4-1/(8\log P)-0.05625/\log^2 P$（K3''''）
...
- K3'''' bound：$3P/4-P/(8\log P)-0.05625P/\log^2 P$
- H\_P 需要：$0$
...
## 附录 L：K3'''' 系数自我审计纠错与 Dusart 上界进一步收紧（2026-05-23 第十一轮）

### L.1 自我审计：K3'''' 系数因子 2 错误

**发现**：附录 H 的 K3'''' 推导第 4-5 步存在**算术错误**：
...
|---|---|---|---|
| 503 | $365.34$ | $363.62$ | $1.72$ |
| 1009 | $735.10$ | $732.14$ | $2.96$ |
| 4999 | $3666.50$ | $3657.22$ | $9.28$ |
| 10007 | $7349.61$ | $7335.77$ | $13.84$ |

### L.3 K3*-three-term-corrected（列方向）
...
### L.4 K3-united-three-term-corrected

$$
|E(P)|+|E^*(P)|<\frac{3P}{2}-\frac{P}{4\log P}-\frac{0.225\,P}{\log^2 P}.
$$

### L.5 引入 Dusart 2010 上界 $\pi(x)\le x/(\log x-1.1)$
...
对 $P\ge 60184$ 有 $1.1/\log P\le 1.1/\log 60184=1.1/11.0=0.1$，故几何级数展开 $1/(1-u)\le 1+u+u^2/(1-u)$：
$$
...
### L.6 定理 K3''''-Dusart-upper（最强外部引理版）

**陈述**：对每个素数 $P\ge 60184$，
$$
|E(P)|<\frac{3P}{4}-\frac{P}{8\log P}-\frac{0.1125\,P}{\log^2 P}-\frac{1}{2}+\frac{0.55}{\log P}+O\!\Bigl(\frac{1}{\log^2 P}\Bigr).
$$

**证明**：用 Dusart 2010 三项下界对 $\pi(P^2)$ 与 Dusart 2010 两项上界对 $\pi(P)$：

$$
\pi(P^2)-\pi(P)\ge \Bigl[\frac{P^2}{2\log P}+\frac{P^2}{4\log^2 P}+\frac{0.225P^2}{\log^3 P}\Bigr]-\Bigl[\frac{P}{\log P}+\frac{1.1P}{\log^2 P}\Bigr].
$$

$(P-1-|E|)\cdot 2P/\log P$ 大于等于上式。除以 $2P/\log P$：

$$
P-1-|E(P)|\ge \frac{P}{4}+\frac{P}{8\log P}+\frac{0.1125P}{\log^2 P}-\frac{1}{2}-\frac{0.55}{\log P}.
$$

故 $|E(P)|\le \frac{3P}{4}-\frac{P}{8\log P}-\frac{0.1125P}{\log^2 P}-\frac{1}{2}+\frac{0.55}{\log P}$ for $P\ge 60184$. $\square$
...
| K3''''-Dusart-upper | $P\ge 60184$ | Dusart 2010 两项 | $-1/2+0.55/\log P\approx -0.45$ |

K3''''-Dusart-upper 在 $P\ge 60184$ 提供额外 $\sim 0.08$ 的常数节省。

### L.8 黎曼/欧拉/高斯式自我审计的意义
...
本附录纠正了 §H 中 K3'''' 系数的因子 2 错误——**改进了一倍真实节省**，
同时引入 Dusart 2010 上界进一步收紧常数。这是**真正的非循环推进**：
不是把已知定理改名，而是**逐行重新计算**发现错误并修正。

### L.9 数值核对

修正后的 K3''''-corrected 上界**严格**收紧，故先前数值核对（满足 K3'''' 旧上界）**仍然**通过新上界（因为 $0.05625<0.1125$，旧上界更松，新上界更严）。

实际数值：
- 旧 K3'''' 给 $P=4999$ 时上界 $\approx 3666.50$
- 新 K3''''-corrected 给 $P=4999$ 时上界 $\approx 3657.22$
- 实际 $|E(4999)|=0$ $\ll$ 二者
