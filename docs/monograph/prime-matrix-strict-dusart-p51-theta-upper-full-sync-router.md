...
**状态：** `dusart_p51_theta_upper_full_external_lane_closed_self_contained_finite_table_remains_open`

Dusart P5.1 的全段外部路线已经闭合：低段由 `theta(x)<x` 到 `8e11` 给出，中段由 `psi<1.00002841x` 与 `psi-theta>0.9999sqrt(x)` 拼接给出，高尾由 FK b0=28 的 `0.00001262<1/36260` 给出。因此可登记 `theta(x)-x<x/36260` 对所有 `x>0` 成立。但严格作者侧完全自足版仍剩 table_012 原始有限计算/hash。
...
| `LowSegmentThetaLtIdentityTo8e11` | `true` | `true` | 对 0<x<=8e11，theta(x)<x，故 theta(x)-x<0<x/36260。 | external table lane |
...
| `DusartP51ThetaUpperFullSyncLedger` | `true` | `true` | 接受 FK 高尾与 Dusart table_012 外部表时，Dusart P5.1 全段 theta(x)-x<x/36260 已闭合。 | external lane theorem closed |
| `DusartP51ThetaUpperFullSelfContainedLedger` | `false` | `false` | 严格作者侧完全自足版仍未闭合，因为 theta table_012 原始计算/hash 未内化。 | internalize table_012 finite computation/hash, and optionally FK kernel source |
