...
| H4 | PDEC/SAE/Rankin | 拆成 `PDEC exclusion`、`SAE local escape exclusion`、Rankin certificates 三项；新增 H4-PDEC 证书模板、首批约束来源引理和约束准入表，固定 `PDEC-Explicit-Cert/PDEC-Dual-Cert` 的输入、下界、约束合法性、准入等级和失败回流规则 | 最终硬输入；PDEC 格式/来源/准入规则闭合但证书未填 |
