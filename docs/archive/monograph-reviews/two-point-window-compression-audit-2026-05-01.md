...
本轮补正澄清了一个逻辑口径：`w=2` 推出孪生素数猜想并不是否定证明的理由。若证明链确实独立、逐行、无缺口，则可以升级为无条件定理。随后 I1 固定阶局部交叉复杂度已被内联证明；当前仍保持条件状态的原因是：二次筛章节依赖的剩余三项输入尚未全部内联为独立验收的无条件证明。
...
> 在 I1 固定阶复杂度引理和剩余三项二次筛输入均成立的条件下，后半方阵中的任意连续窗口可从约 `P/2` 行压缩到  
