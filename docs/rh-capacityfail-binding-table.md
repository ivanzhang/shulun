...
1. **DGap 投影与低维抽取**：frame 上界与逐步正交投影补强见 `docs/rh-dgap-projection-line-by-line-audit.md`；低维抽取逐行补强见 `docs/rh-dgap-lowdim-extraction-line-by-line-audit.md`；DSO/PI 桥接见 `docs/rh-dso-pi-squarefunction-bridge-audit.md`；FCT_seed 同型匹配见 `docs/rh-fct-seed-isomorphism-audit.md`；盒模板尾项统一见 `docs/rh-fourier-vaaler-tail-uniform-audit.md`；FCT closure 无无限递归见 `docs/rh-fct-closure-no-cycle-final-audit.md`；剩余为上游常数标准化与全局无回流审查；
