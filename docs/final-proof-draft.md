...
矩阵的含义是：每一个可能破坏端点平均的项，若不是普通 sawtooth 相位和，就落入 6.14.1 六类坏层；若是普通 sawtooth 相位和，则由 6.7--6.13 的 NL/FNL、UAS、四点能量与离散 coarea 链条控制。
...
本节把“最后单一接口”从素数权 Type-I/AP 问题改回硬窗口 sawtooth 问题，并给出可审查覆盖矩阵。若 6.7--6.15 中 UAS、四点能量、离散 coarea 与 DBA-closure 的证明按当前严写版本逐项通过，则 `B.0.4*` 闭合，行方向不再依赖 `B.0.4S-short`。若审查发现某个边界项既不能规范化为 sawtooth 相位和、又不属于 6.14 六类坏层，则该项即为新的唯一剩余缺口。
...
这条链条已经把 sawtooth 相消的所有逻辑接口显式化。经过 6.18.1d--e 与 6.18.2b 的补强，`LV` 的权重/步长问题和 `AE` 的层化损失都已接入 6.14--6.15 的坏层与参数账本。当前最后审稿点集中为：6.13--6.14 对所有 coarea、Jacobian、ramification 与 resultants 坏层是否覆盖无遗漏。该点通过后，硬窗口行命题的相消接口才可声称无条件闭合；若失败，失败项就是唯一剩余缺口。
...
| resultant atlas | 6.14.2a--b 有限清单与穷尽性反证 | 已结构闭合，需逐项审稿核对 |
| FGH 高度账本 | 6.14.3a、6.14.4a 高度传播与坏素权重 | 已降为标准高度/resultant 常数复核 |
| 参数余量 | 6.15 将 `LV/AE/coarea/DBA` 损失纳入 `C_*` | 已给显式对数余量方案 |
| 行命题硬路线 | `B.0.4*` + 第 4 节窗口安全间隙 | 理论上随上述复核闭合 |
| 列命题 | 第 5 节高阈值列场、容量比较、坏列整数化 | 仍需最终逐常数复核 |
| 全体奇素数 | 定理 6.2--6.3 有限验证接口 | 需显式 `P_*` 后运行完整有限验证 |

**最终审稿结论。** 硬窗口 sawtooth 主线已经从“新解析输入”转化为有限清单复核：atlas 无遗漏、FGH 高度账本、参数余量、列方向常数和显式阈值。除显式阈值与有限小素数验证外，当前主线的结构性缺口已被压缩为可逐项审稿的有限表格。论文若要声明“所有奇素数”版本，仍必须抽取显式 `P_*` 并执行附录 D 的有限验证；在此之前，严格表述应为“所有充分大奇素数”的硬路线证明框架。
...
**当前阻塞点。** 文稿中仍有大量 `o(1)`、`O(1)`、`P^{o(1)}`、`\ll` 和“足够大”表述；它们足以支持渐近理论框架，但尚未给出可计算数值阈值。因此本轮不能诚实地产生实际 `P_*`，只能给出抽取账本并完成 `P_{max}=10000` 的扩展验证。任何直接声明“所有奇素数已无条件闭合”都需要先完成本节账本的数值化。
...
由 6.17 的 admissibility 核查与 6.18 的 sawtooth 相消链条，B.0.4\* 的证明被压缩为 atlas 穷尽性、有限生成高度账本和最终参数余量复核。若这些审稿级复核通过，B.0.4 的其余部分严格闭合；若复核失败，则失败项即为唯一剩余接口。
