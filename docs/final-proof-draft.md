...
的 Jacobian 退化意味着水平面族具有异常大的奇异分支。若存在三维以上正密度分支，则局部可把 `a_4` 写成函数
...
\[
R(\Phi(a_1,a_2,a_3))=R(a_2)+R(a_3)-R(a_1).
\]

对三个变量求偏导，得到

\[
R'(\Phi)\partial_{a_1}\Phi=-R'(a_1),
\quad
R'(\Phi)\partial_{a_2}\Phi=R'(a_2),
\quad
R'(\Phi)\partial_{a_3}\Phi=R'(a_3).
\]

混合偏导相容性给出

\[
\partial_{a_i}\left(\frac{R'(a_j)}{R'(\Phi)}\right)
=
\partial_{a_j}\left(\frac{R'(a_i)}{R'(\Phi)}\right).
\]
...
的分离变量方程。该方程迫使 `L` 具有 Möbius 型形式

\[
L(x)=\frac{c_1}{c_2x+c_3}
\]

或常数型。积分后，`R` 只能是仿射、指数型或 Möbius 型；在有理函数类中，指数型不可能，因此只剩仿射或 Möbius 退化。

这给出 JND 的可证路线：把 Jacobian 三维奇异分支转化为上述偏微分相容方程，再用一维有理函数的对数导数分类排除非退化情形。

### 6.12.2 有理函数分类引理

**引理 JND-class（有理四点奇异分类）。** 设 `R\in\mathbb C(x)` 非常值。若四点关系

\[
R(x_1)-R(x_2)-R(x_3)+R(x_4)=0
\]

的真四点部分存在三维奇异分支，且该分支不包含配对对角，则 `R` 在 Möbius 变换复合下为仿射函数或一次分式函数。

**证明纲要。** 在奇异分支的光滑点，用隐函数定理写出 `x_4=\Phi(x_1,x_2,x_3)`。偏导相容性推出 `R''/R'` 满足分离变量恒等式。因为 `R''/R'` 是有理函数，该恒等式迫使其极点至多一个且为一阶；积分后 `R'` 为单项幂型，`R` 为幂函数或 Möbius 型。四点加法关系排除高次幂函数，只留下仿射与一次分式。仿射由二阶场非恒等排除，一次分式由 DBA 判别式层吸收。

该引理是 JND 的核心代数几何命题。它看起来可证，但需要在最终论文中把“奇异分支光滑点”“配对对角排除”“高次幂排除”逐项严写。
...
### 6.12.4 FGH 与 DBA-closure 的闭合

由 6.12.3，对固定 connected skeleton，所有判别式高度损失为 `\log^{O_r}P`。对 skeleton、局部 Bell 选择和素数标签求和时，引理 2.7 已提供
...
这意味着最后真正非平凡的数学障碍主要集中在 JND-class；FGH 更像技术账本，原则上可在附录中完全展开。

### 6.12.5 当前最接近闭合的形式

若 JND-class 严格证明完成，则 JND 成立；由 6.12.3--6.12.4，FGH 成立；于是 DBA-closure 与 4E-DISP 成立；结合 6.10 的局部 rank 界得到四点能量定理；再推出 UAS，最终闭合 B.0.4S-short 条件链条。

因此当前唯一真正硬核的剩余点是：严写并证明 JND-class 的有理函数四点奇异分类。若该分类无例外成立，整条 UAS/DBA 路线将从条件归约变为实际证明链条。
