...
**命题 AE（arc concentration to four-point energy）。** 若某个 `F_t` 在短弧 `J` 上有

\[
M=\#\{a\asymp A:F_t(a)\in J\}
\]

个命中点，且 `M` 超过 UAS 预测量 `A\theta(Q')+|J|A+P^{o(1)}`，则四点场
...
在厚度 `O(|J|)` 内拥有超过随机尺度的解数。

**证明。** 命中值全部落在同一短弧内，所以对命中集合 `A_J`，任意三元组 `(a_1,a_2,a_3)` 至少有约 `M^4/L` 个四元组落在同一离散层，其中 `L\ll |J|A+P^{o(1)}` 是短弧内可分辨层数。等价地，Balog--Szemerédi 型能量下界给

\[
E_4(A_J)\gg {M^4\over |J|A+P^{o(1)}}.
\]

这些四元组满足 `|\mathcal E|\ll |J|`。清除分母后正是 6.10--6.13 的四点厚化方程。对角和半对角部分只有 `O(M^2A^{o(1)})`，不足以支持超阈值 `M`；因此若 UAS 失败，必有真四点厚化能量异常。
...
这条链条已经把 sawtooth 相消的所有逻辑接口显式化。当前最后审稿点只剩：LV 中加权 van der Corput 的常数、AE 中短弧层数 `L\ll |J|A+P^{o(1)}`、以及 6.13--6.14 对所有 coarea 坏层的覆盖。三者全部严写通过后，硬窗口行命题的相消接口无条件闭合；若任一项失败，则失败项就是唯一剩余缺口。
