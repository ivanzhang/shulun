...
**证明。** 引理 6.18.2b 给出至少 `M^4/L^3` 个厚化四元组。对角与半对角四元组至多 `O(M^2A^{o(1)})`，已在 6.10.2 单独计数；分母不可逆、导数小、Jacobian 退化和 ramification 壳进入 6.14 的 DBA-closure。剔除这些层后，剩余四元组是真四点厚化解，清分母后正是 6.10--6.13 的四点场方程。若其数量超过四点 rank/coarea 上界，则得到 4E-DISP 禁止的异常。

**审查结论。** 原先粗略的 `M^4/L` 应替换为安全的 `M^4/L^3`。在主应用中 `L=O(1)` 或 `L\le\log^{O(1)}P`，该损失由 6.15 的参数余量吸收；若某个分片导致 `L` 超过对数幂，则这不是组合能量问题，而是新的分母/ramification 层，必须回到 `DBA-closure` 覆盖矩阵。
