...
这一定式比总变差强得多；它要求随 `a` 变化的倒数相位 `N/a` 在 Stieltjes 权上产生平均相消。本文主线用 6.7--6.17 的 USC connected 机制、UAS/DBA、四点能量与离散 coarea 推出该 sawtooth 界，从而闭合硬路线 B.0.4*。反之，若该 sawtooth 界在最终审稿中失败，短切片逐点端点误差无法被总变差论证消除。
