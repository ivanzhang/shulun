...
| C6 no-cycle | 将事件图势函数下降写成形式化图论引理 | 已有摘要，仍需编号化 |
| C9 tail closure | 将 Vaaler/Fourier 尾项链逐项定理化 | 已有摘要，仍需编号化 |
...
本轮不能完成的是：把主定理升级为最终无条件 RH 定理。下一步最优任务应转向 C4/C5/C6/C9 的单篇内联编号化，先从最短且最容易形式化的 C6 no-cycle 开始。
