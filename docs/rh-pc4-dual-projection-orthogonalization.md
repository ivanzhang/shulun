...
逐行强度审查与 frame/逐步正交投影补强见 `docs/rh-dgap-projection-line-by-line-audit.md`。
...
因此 `V` 可按逐步正交投影分解到三个出口子空间：先投影到 `V_PI`，再在正交余空间中投影到 `V_low`，最后剩余为 `V_err`。逐行论证见 `docs/rh-dgap-projection-line-by-line-audit.md`。其中：
...
**证明。** 由第 3 节，分散正质量给盒空间投影能量下界。更精确地，由 `DGap-Projection-LineByLine` 的 frame 上界、常数方向剥离与 Cauchy 下界得到 `||P_Vh||_2^2>=X^{2β-1-o(1)}`。再按逐步正交投影分配到 `V_PI`、`V_low` 与 `V_err`。若允许投影部分承载固定比例能量，得 1；否则能量主要落入低维频率、低体积/短簇或误差逃逸，分别给 2、3、4。证毕。
