...
单篇合并雏形已写入 `docs/rh-merged-proof-draft-v1.md`。该稿按线性论文顺序整合 PC1、PC2、统一覆盖场方程、过疏 PC3/OV2、过密 Dual/DGap、内部终端、外部吸收、Fourier/Vaaler 尾项、EXT 外部定理包和合并主定理。

该稿仍明确标注为“合并稿条件版”：主定理仍以主文链、容量引理和 `EXT-*` 外部定理最终编号/引用成立为前提。下一步必须把 `docs/...` 交叉引用替换为内部编号定理或正式文献，并逐条内联 `CapacityFail` 容量语句。
