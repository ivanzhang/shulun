...
Using the fixed-order complexity lemma above, assume the following three remaining inputs hold with the constants recorded in the secondary-sieve research note:
\begin{enumerate}
  \item the adaptive true-hit layer decomposition is used, with any single-prime concentration routed to a Single-Prime CRTDefect exit;
  \item the SC2 second-correlation estimate controls the two-line near-crossing error and the $45^\circ$ main-synchronization peak, while the finite package for primes $q\le100$ is checked separately;
  \item the weighted Bonferroni zero-mass estimate and smooth-to-sharp cutoff return hold with the recorded endpoint loss.
\end{enumerate}
...
The current audit is archived under \path{docs/archive/monograph-reviews/} and classifies this as a research proposition conditional on the three inputs above.  The fixed-order complexity input has been inlined as the preceding lemma.  The fact that an unconditional proof would imply fixed-gap prime-pair consequences, including the twin-prime case when $w=2$, is not a logical obstruction.  It only marks the strength of the claim.  The reason this chapter remains conditional is that the three remaining inputs have not yet all been inlined as independently verified unconditional proofs.
\end{proof}

\begin{proposition}[Window-compressed conditional variant]
Keep the same fixed even integer $w>0$ and the same three remaining inputs as in the preceding proposition.  Let $I$ be any consecutive back-half window of $H(P)$ rows in the $P\times P$ matrix, so that $|I|=H(P)P+O(P)$.  By the fixed-order complexity lemma, write
\[
